...
static HOPreciseTimer *__sharedInstance = nil;
...
@implementation HOPreciseTimer

+ (void)scheduleAction:(SEL)action target:(id)target inTimeInterval:(NSTimeInterval)timeInterval {
    if ( !__sharedInstance ) __sharedInstance = [[HOPreciseTimer alloc] init];
...
void *thread_entry(void* argument) {
    [(__bridge HOPreciseTimer*)argument thread];
    return NULL;
}
    ...
    while ( 1 ) {
        ...
        @autoreleasepool {
            ...
            if ( (double)(mach_absolute_time() * timebase_ratio) >= time-kSpinLockTime ) {
                ...
                printf("HOPreciseTimer fired: %lfs time discrepancy\n", discrepancy);
#endif
