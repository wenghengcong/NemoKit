...
typedef void(^NKDelayedBlockHandle)(BOOL cancel);

static NKDelayedBlockHandle perform_block_after_delay(CGFloat seconds, dispatch_block_t block) {
    ...
    __block NKDelayedBlockHandle delayHandleCopy = nil;
    
    NKDelayedBlockHandle delayHandle = ^(BOOL cancel){
        ...
        // Doing this here means that if the block is canceled, we aren't NKlding onto retained objects for any longer than necessary.
...
static void cancel_delayed_block(NKDelayedBlockHandle delayedHandle) {
