...
{
    void (^block)(void) = [inBlock copy];
    id ret = [self timerWithTimeInterval:inTimeInterval target:self selector:@selector(nk_p_ExecuteBlock:) userInfo:block repeats:inRepeats];
    return ret;
}

+(void)nk_p_ExecuteBlock:(NSTimer *)inTimer;
{
    if([inTimer userInfo])
        {
