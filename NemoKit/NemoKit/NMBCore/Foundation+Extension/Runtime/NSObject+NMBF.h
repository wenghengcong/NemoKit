...
@interface NSObject (NMBF)


/// 判断当前类是否有重写某个父类的指定方法
/// @param selector 要比较的方法
...
- (nullable id)nmbf_performSelectorToSuperclass:(SEL)aSelector;
...
///  @code
///  CGFloat alpha;
///  [view nmbf_performSelector:@selector(alpha) withPrimitiveReturnValue:&alpha];
///  @endcode
- (void)nmbf_performSelector:(SEL)selector
...
///  [control nmbf_performSelector:@selector(addTarget:action:forControlEvents:) withArguments:&target, &action, &events, nil];
///  @endcode
- (nullable id)nmbf_performSelector:(SEL)selector
...
/// @param block block  用于遍历的 block
+ (void)nmbf_enumrateIvarsOfClass:(Class)aClass
               includingInherited:(BOOL)includingInherited
                       usingBlock:(void (^)(Ivar ivar, NSString *ivarDescription))block;


/// 使用 block 遍历指定 class 的所有属性，不包含 superclasses 里定义的 property
/// @param block block 用于遍历的 block，如果要获取 property 的信息，推荐用 NMUIPropertyDescriptor。
/// TODO: NMUIPropertyDescriptor
...
+ (void)nmbf_enumratePropertiesOfClass:(Class)aClass
                    includingInherited:(BOOL)includingInherited
                            usingBlock:(void (^)(objc_property_t property, NSString *propertyName))block;



/// 使用 block 遍历当前实例的所有方法，不包含 superclasses 里定义的 method
/// @param block block  用于遍历的 block
- (void)nmbf_enumrateInstanceMethodsUsingBlock:(void (^)(Method method, SEL selector))block;
...
/// @param block block  用于遍历的 block
+ (void)nmbf_enumrateInstanceMethodsOfClass:(Class)aClass
                        includingInherited:(BOOL)includingInherited
                                usingBlock:(void (^)(Method method, SEL selector))block;
...
/// @param block block  用于遍历的 block
+ (void)nmbf_enumerateProtocolMethods:(Protocol *)protocol
...
/// @note NMUI 提供2种方式兼容系统的 access prohibited 异常：
/// 1. 通过将配置表的 IgnoreKVCAccessProhibited 置为 YES 来全局屏蔽系统的异常警告，代码中依然正常使用系统的 valueForKey:、setValue:forKey:，当开启后再遇到 access prohibited 异常时，将会用 NMUIWarnLog 来提醒，不再中断 App 的运行，这是首选推荐方案。
/// 2. 使用 nmbf_valueForKey:、nmbf_setValue:forKey: 代替系统的 valueForKey:、setValue:forKey:，适用于不希望全局屏蔽，只针对某个局部代码自己处理的场景。
...
- (nullable id)nmbf_valueForKey:(NSString *)key;
...
/// 2. 使用 nmbf_valueForKey:、nmbf_setValue:forKey: 代替系统的 valueForKey:、setValue:forKey:，适用于不希望全局屏蔽，只针对某个局部代码自己处理的场景。
/// @link https://github.com/Tencent/QMUI_iOS/issues/617
/// /// @value key 对应的 value，如果该 key 原本是非对象的值，会被用 NSNumber、NSValue 包裹后返回
/// @param key  ivar 属性名，支持下划线或不带下划线
- (void)nmbf_setValue:(nullable id)value forKey:(NSString *)key;

@end

@interface NSObject (NMBF_DataBind)
...
/*
 ...
 @code
     ...
     [cell nmbf_bindObject:indexPath forKey:@"indexPath"];
     }
     
     - (void)didTapButton:(UIButton *)button {
     // 2）在这里取出被点击的 button 的 indexPath 对象
     NSIndexPath *indexPathTapped = [button nmbf_getBoundObjectForKey:@"indexPath"];
     }
 @endcode
 */
...
/// @param object 绑定的对象
/// @param key 绑定的key
- (void)nmbf_bindObject:(nullable id)object forKey:(NSString *)key;

/// 给对象绑定上另一个对象以供后续取出使用，但相比于 nmbf_bindObject:forKey:，该方法不会 strong 强引用传入的 object
- (void)nmbf_bindObjectWeakly:(nullable id)objcet forKey:(NSString *)key;

///  取出之前使用 bind 方法绑定的对象
- (nullable id)nmbf_getBoundObjectForKey:(NSString *)key;
