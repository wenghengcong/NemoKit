...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        NMBFExtendImplementationOfNonVoidMethodWithoutArguments([UIImage class], @selector(description), NSString *, ^NSString *(UIImage *selfObject, NSString *originReturnValue) {
            return ([NSString stringWithFormat:@"%@, scale = %@", originReturnValue, @(selfObject.scale)]);
        });
        
        NMBFOverrideImplementation([UIImage class], @selector(resizableImageWithCapInsets:resizingMode:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^UIImage *(UIImage *selfObject, UIEdgeInsets capInsets, UIImageResizingMode resizingMode) {
                ...
                if (!CGSizeIsEmpty(selfObject.size) && (UIEdgeInsetsGetHorizontalValue(capInsets) >= selfObject.size.width || UIEdgeInsetsGetVerticalValue(capInsets) >= selfObject.size.height)) {
                    ...
                    NMBFLogWarn(@"UIImage (NMUI)", @"UIImage (NMUI) resizableImageWithCapInsets 传进来的 capInsets 的水平/垂直方向的和应该小于图片本身的大小，否则会导致 render 时出现 invalid context 0x0 的错误。");
                }
...
+ (UIImage *)nmui_imageWithSize:(CGSize)size opaque:(BOOL)opaque scale:(CGFloat)scale actions:(void (^)(CGContextRef contextRef))actionBlock {
...
- (UIImage *)nmui_grayImage {
    // CGBitmapContextCreate 是无倍数的，所以要自己换算成1倍
    CGSize size = self.nmui_sizeInPixel;
    ...
    if (self.nmui_opaque) {
        ...
        // 用 CGBitmapContextCreateImage 方式创建出来的图片，CGImageAlphaInfo 总是为 CGImageAlphaInfoNone，导致 nmui_opaque 与原图不一致，所以这里再做多一步
        grayImage = [UIImage nmui_imageWithSize:grayImage.size opaque:NO scale:grayImage.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)nmui_imageWithAlpha:(CGFloat)alpha {
    return [UIImage nmui_imageWithSize:self.size opaque:NO scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawInRect:CGRectMakeWithSize(self.size) blendMode:kCGBlendModeNormal alpha:alpha];
    }];
}

- (UIImage *)nmui_imageWithTintColor:(UIColor *)tintColor {
    ...
    //    }
    //#endif
    return [UIImage nmui_imageWithSize:self.size opaque:self.nmui_opaque scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)nmui_imageWithBlendColor:(UIColor *)blendColor {
    UIImage *coloredImage = [self nmui_imageWithTintColor:blendColor];
...
- (UIImage *)nmui_imageWithImageAbove:(UIImage *)image atPoint:(CGPoint)point {
    return [UIImage nmui_imageWithSize:self.size opaque:self.nmui_opaque scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)nmui_imageWithSpacingExtensionInsets:(UIEdgeInsets)extension {
    ...
    return [UIImage nmui_imageWithSize:contextSize opaque:self.nmui_opaque scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawAtPoint:CGPointMake(extension.left, extension.top)];
    }];
}

- (UIImage *)nmui_imageWithClippedRect:(CGRect)rect {
...
- (UIImage *)nmui_imageWithClippedCornerRadius:(CGFloat)cornerRadius {
    return [self nmui_imageWithClippedCornerRadius:cornerRadius scale:self.scale];
}

- (UIImage *)nmui_imageWithClippedCornerRadius:(CGFloat)cornerRadius scale:(CGFloat)scale {
    if (cornerRadius <= 0) {
        return self;
    }
    return [UIImage nmui_imageWithSize:self.size opaque:NO scale:scale actions:^(CGContextRef contextRef) {
        [[UIBezierPath bezierPathWithRoundedRect:CGRectMakeWithSize(self.size) cornerRadius:cornerRadius] addClip];
        [self drawInRect:CGRectMakeWithSize(self.size)];
    }];
}

- (UIImage *)nmui_imageResizedInLimitedSize:(CGSize)size {
    return [self nmui_imageResizedInLimitedSize:size resizingMode:NMUIImageResizingModeScaleAspectFit];
}

- (UIImage *)nmui_imageResizedInLimitedSize:(CGSize)size resizingMode:(NMUIImageResizingMode)resizingMode {
    return [self nmui_imageResizedInLimitedSize:size resizingMode:resizingMode scale:self.scale];
}

- (UIImage *)nmui_imageResizedInLimitedSize:(CGSize)size resizingMode:(NMUIImageResizingMode)resizingMode scale:(CGFloat)scale {
    ...
    if (resizingMode >= NMUIImageResizingModeScaleAspectFit && resizingMode <= NMUIImageResizingModeScaleAspectFillBottom) {
        CGFloat horizontalRatio = size.width / imageSize.width;
        CGFloat verticalRatio = size.height / imageSize.height;
        CGFloat ratio = 0;
        if (resizingMode >= NMUIImageResizingModeScaleAspectFill && resizingMode < (NMUIImageResizingModeScaleAspectFill + 10)) {
            ratio = MAX(horizontalRatio, verticalRatio);
        } else {
            // 默认按 NMUIImageResizingModeScaleAspectFit
        ...
        // 默认按照 NMUIImageResizingModeScaleToFill
        drawingRect = CGRectMakeWithSize(size);
        contextSize = size;
    }
    
    return [UIImage nmui_imageWithSize:contextSize opaque:self.nmui_opaque scale:scale actions:^(CGContextRef contextRef) {
        [self drawInRect:drawingRect];
    }];
}

- (UIImage *)nmui_imageWithOrientation:(UIImageOrientation)orientation {
    ...
    return [UIImage nmui_imageWithSize:contextSize opaque:NO scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)nmui_imageWithBorderColor:(UIColor *)borderColor path:(UIBezierPath *)path {
    if (!borderColor) {
        return self;
    }
...
- (UIImage *)nmui_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth cornerRadius:(CGFloat)cornerRadius {
    return [self nmui_imageWithBorderColor:borderColor borderWidth:borderWidth cornerRadius:cornerRadius dashedLengths:0];
}

- (UIImage *)nmui_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth cornerRadius:(CGFloat)cornerRadius dashedLengths:(const CGFloat *)dashedLengths {
    ...
    return [self nmui_imageWithBorderColor:borderColor path:path];
}

- (UIImage *)nmui_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth borderPosition:(NMUIImageBorderPosition)borderPosition {
    if (borderPosition == NMUIImageBorderPositionAll) {
        return [self nmui_imageWithBorderColor:borderColor borderWidth:borderWidth cornerRadius:0];
    } else {
        // TODO 使用bezierPathWithRoundedRect:byRoundingCorners:cornerRadii:这个系统接口
        UIBezierPath* path = [UIBezierPath bezierPath];
        if ((NMUIImageBorderPositionBottom & borderPosition) == NMUIImageBorderPositionBottom) {
        ...
        if ((NMUIImageBorderPositionTop & borderPosition) == NMUIImageBorderPositionTop) {
            [path moveToPoint:CGPointMake(0, borderWidth / 2)];
            [path addLineToPoint:CGPointMake(self.size.width, borderWidth / 2)];
        }
        if ((NMUIImageBorderPositionLeft & borderPosition) == NMUIImageBorderPositionLeft) {
            [path moveToPoint:CGPointMake(borderWidth / 2, 0)];
            [path addLineToPoint:CGPointMake(borderWidth / 2, self.size.height)];
        }
        if ((NMUIImageBorderPositionRight & borderPosition) == NMUIImageBorderPositionRight) {
...
- (UIImage *)nmui_imageWithMaskImage:(UIImage *)maskImage usingMaskImageMode:(BOOL)usingMaskImageMode {
...
+ (UIImage *)nmui_animatedImageWithData:(NSData *)data {
    return [self nmui_animatedImageWithData:data scale:1];
}

+ (UIImage *)nmui_animatedImageWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self nmui_frameDurationAtIndex:i source:source];
...
+ (float)nmui_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
...
+ (UIImage *)nmui_animatedImageNamed:(NSString *)name {
    return [UIImage nmui_animatedImageNamed:name scale:1];
}

+ (UIImage *)nmui_animatedImageNamed:(NSString *)name scale:(CGFloat)scale {
    ...
    return [UIImage nmui_animatedImageWithData:data scale:scale];
}

+ (UIImage *)nmui_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size path:(UIBezierPath *)path addClip:(BOOL)addClip {
    size = CGSizeFlatted(size);
    return [UIImage nmui_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
...
+ (UIImage *)nmui_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth cornerRadius:(CGFloat)cornerRadius {
    ...
    return [UIImage nmui_imageWithStrokeColor:strokeColor size:size path:path addClip:NO];
}

+ (UIImage *)nmui_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth borderPosition:(NMUIImageBorderPosition)borderPosition {
    CGContextInspectSize(size);
    if (borderPosition == NMUIImageBorderPositionAll) {
        return [UIImage nmui_imageWithStrokeColor:strokeColor size:size lineWidth:lineWidth cornerRadius:0];
    } else {
        // TODO 使用bezierPathWithRoundedRect:byRoundingCorners:cornerRadii:这个系统接口
        UIBezierPath* path = [UIBezierPath bezierPath];
        if ((NMUIImageBorderPositionBottom & borderPosition) == NMUIImageBorderPositionBottom) {
...
+ (UIImage *)nmui_imageWithColor:(UIColor *)color {
    return [UIImage nmui_imageWithColor:color size:CGSizeMake(4, 4) cornerRadius:0];
}

+ (UIImage *)nmui_imageWithColor:(UIColor *)color size:(CGSize)size cornerRadius:(CGFloat)cornerRadius {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    
    color = color ? color : UIColorClear;
    BOOL opaque = (cornerRadius == 0.0 && [color nmui_alpha] == 1.0);
    return [UIImage nmui_imageWithSize:size opaque:opaque scale:0 actions:^(CGContextRef contextRef) {
...
+ (UIImage *)nmui_imageWithColor:(UIColor *)color size:(CGSize)size cornerRadiusArray:(NSArray<NSNumber *> *)cornerRadius {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    ...
    return [UIImage nmui_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        ...
        UIBezierPath *path = [UIBezierPath nmui_bezierPathWithRoundedRect:CGRectMakeWithSize(size) cornerRadiusArray:cornerRadius lineWidth:0];
        [path addClip];
        [path fill];
    }];
}

+ (UIImage *)nmui_imageWithShape:(NMUIImageShape)shape size:(CGSize)size lineWidth:(CGFloat)lineWidth tintColor:(UIColor *)tintColor {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    ...
    return [UIImage nmui_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        ...
        switch (shape) {
            ...
            case NMUIImageShapeNavBack: {
                ...
                break;
            case NMUIImageShapeDisclosureIndicator: {
                drawByStroke = YES;
                path = [UIBezierPath bezierPath];
                path.lineWidth = lineWidth;
        ...
        if (shape == NMUIImageShapeDetailButtonImage) {
...
+ (UIImage *)nmui_imageWithShape:(NMUIImageShape)shape size:(CGSize)size tintColor:(UIColor *)tintColor {
    CGFloat lineWidth = 0;
    switch (shape) {
        case NMUIImageShapeNavBack:
            lineWidth = 2.0f;
    ...
    return [UIImage nmui_imageWithShape:shape size:size lineWidth:lineWidth tintColor:tintColor];
}

+ (UIImage *)nmui_imageWithAttributedString:(NSAttributedString *)attributedString {
    ...
    return [UIImage nmui_imageWithSize:stringSize opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        [attributedString drawInRect:CGRectMakeWithSize(stringSize)];
    }];
}

+ (UIImage *)nmui_imageWithView:(UIView *)view {
    CGContextInspectSize(view.bounds.size);
    return [UIImage nmui_imageWithSize:view.bounds.size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        [view.layer renderInContext:contextRef];
    }];
}

+ (UIImage *)nmui_imageWithView:(UIView *)view afterScreenUpdates:(BOOL)afterUpdates {
