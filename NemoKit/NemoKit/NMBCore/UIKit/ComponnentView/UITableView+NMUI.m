...
@implementation UITableView (NMUI)

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        NMBFExtendImplementationOfNonVoidMethodWithTwoArguments([UITableView class], @selector(initWithFrame:style:), CGRect, UITableViewStyle, UITableView *, ^UITableView *(UITableView *selfObject, CGRect frame, UITableViewStyle style, UITableView *originReturnValue) {
            // iOS 11 之后 estimatedRowHeight 如果值为 UITableViewAutomaticDimension，estimate 效果也会生效（iOS 11 以前要 > 0 才会生效）。
            // 而当使用 estimate 效果时，会导致 contentSize 之类的计算不准确，所以这里给一个途径让项目可以方便地控制 NMUITableView（及其子类） 和 UITableView（不包含子类，例如 UIPickerTableView）的 estimatedRowHeight 效果的开关 https://github.com/Tencent/QMUI_iOS/issues/313
            if ([selfObject isKindOfClass:NSClassFromString(@"NMUITableView")] || [NSStringFromClass(selfObject.class) isEqualToString:@"UITableView"]) {
        ...
        NMBFOverrideImplementation([UITableView class], @selector(sizeThatFits:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
        ...
        NMBFOverrideImplementation([UITableView class], @selector(scrollToRowAtIndexPath:atScrollPosition:animated:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UITableView *selfObject, NSIndexPath *indexPath, UITableViewScrollPosition scrollPosition, BOOL animated) {
                ...
                if (!isIndexPathLegal) {
                    NMBFLogWarn(@"UITableView (NMUI)", @"%@ - target indexPath : %@ ，不合法的indexPath。\n%@", selfObject, indexPath, [NSThread callStackSymbols]);
        ...
        if (@available(iOS 13.0, *)) {
            if (@available(iOS 13.1, *)) {
                ...
                NMBFExtendImplementationOfVoidMethodWithoutArguments([UITableViewCell class], @selector(layoutSubviews), ^(UITableViewCell *selfObject) {
                    if ([selfObject.accessoryView isKindOfClass:[UIButton class]]) {
                        ...
                        selfObject.accessoryView.nmui_left = selfObject.nmui_width - defaultRightMargin - selfObject.accessoryView.nmui_width;
                        selfObject.accessoryView.nmui_top = CGRectGetMinYVerticallyCenterInParentRect(selfObject.frame, selfObject.accessoryView.frame);;
                        selfObject.contentView.nmui_right = selfObject.accessoryView.nmui_left;
                    }
                });
            }
        }
...
- (void)nmui_scrollToRowAtIndexPath:(NSIndexPath *)indexPath atScrollPosition:(UITableViewScrollPosition)scrollPosition animated:(BOOL)animated {
    if (!indexPath) {
        return;
    }
    ...
    if (!isIndexPathLegal) {
        ...
        [self nmui_scrollToRowAtIndexPath:indexPath atScrollPosition:scrollPosition animated:animated];
    }
}

- (void)nmui_styledAsNMUITableView {
...
- (NSIndexPath *)nmui_indexPathForRowAtView:(UIView *)view {
    ...
    return [self nmui_indexPathForRowAtView:view.superview];
}

- (NSInteger)nmui_indexForSectionHeaderAtView:(UIView *)view {
...
- (NSArray<NSNumber *> *)nmui_indexForVisibleSectionHeaders {
    ...
    for (NSInteger i = 0; i < visibleSections.count; i++) {
        ...
        if ([self nmui_isHeaderVisibleForSection:section]) {
...
- (NSInteger)nmui_indexOfPinnedSectionHeader {
    NSArray<NSNumber *> *visibleSectionIndex = [self nmui_indexForVisibleSectionHeaders];
    for (NSInteger i = 0; i < visibleSectionIndex.count; i++) {
        NSInteger section = visibleSectionIndex[i].integerValue;
        if ([self nmui_isHeaderPinnedForSection:section]) {
            return section;
        } else {
            continue;
        }
...
- (BOOL)nmui_isHeaderPinnedForSection:(NSInteger)section {
    ...
    BOOL isSectionScrollIntoContentInsetTop = self.contentOffset.y + self.nmui_contentInset.top > CGRectGetMinY(rectForSection);// 表示这个 section 已经往上滚动，超过 contentInset.top 那条线了
    BOOL isSectionStayInContentInsetTop = self.contentOffset.y + self.nmui_contentInset.top <= CGRectGetMaxY(rectForSection) - CGRectGetHeight(rectForHeader);// 表示这个 section 还没被完全滚走
    BOOL isPinned = isSectionScrollIntoContentInsetTop && isSectionStayInContentInsetTop;
    return isPinned;
}

- (BOOL)nmui_isHeaderVisibleForSection:(NSInteger)section {
    if (self.style != UITableViewStylePlain) return NO;
    if (section >= [self numberOfSections]) return NO;
    ...
    BOOL isSectionStayInContentInsetTop = self.contentOffset.y + self.nmui_contentInset.top < CGRectGetMaxY(rectForSection);// 表示这个 section 还没被完全滚走
    BOOL isVisible = isSectionScrollIntoBounds && isSectionStayInContentInsetTop;
    return isVisible;
}

- (NMUITableViewCellPosition)nmui_positionForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSInteger numberOfRowsInSection = [self.dataSource tableView:self numberOfRowsInSection:indexPath.section];
    if (numberOfRowsInSection == 1) {
        return NMUITableViewCellPositionSingleInSection;
    }
    if (indexPath.row == 0) {
        return NMUITableViewCellPositionFirstInSection;
    }
    if (indexPath.row == numberOfRowsInSection - 1) {
        return NMUITableViewCellPositionLastInSection;
    }
    return NMUITableViewCellPositionMiddleInSection;
}

- (BOOL)nmui_cellVisibleAtIndexPath:(NSIndexPath *)indexPath {
    NSArray<NSIndexPath *> *visibleCellIndexPaths = self.indexPathsForVisibleRows;
...
- (void)nmui_scrollToRowFittingOffsetY:(CGFloat)offsetY atIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated {
    [self alertEstimatedHeightUsageIfDetected];
    ...
    if (canScrollRowToTop) {
        ...
        [self nmui_scrollToBottomAnimated:animated];
    }
}

- (CGSize)nmui_realContentSize {
    [self alertEstimatedHeightUsageIfDetected];
...
- (BOOL)nmui_canScroll {
    ...
    if ([self.tableHeaderView isKindOfClass:[UISearchBar class]]) {
        BOOL canScroll = self.nmui_realContentSize.height + UIEdgeInsetsGetVerticalValue(self.nmui_contentInset) > CGRectGetHeight(self.bounds);
        return canScroll;
    } else {
        return [super nmui_canScroll];
    }
}
...
- (void)alertEstimatedHeightUsageIfDetected {
    ...
    if (usingEstimatedRowHeight || usingEstimatedSectionHeaderHeight || usingEstimatedSectionFooterHeight) {
        [self NMUISymbolicUsingTableViewEstimatedHeightMakeWarning];
    }
}

- (void)NMUISymbolicUsingTableViewEstimatedHeightMakeWarning {
    NMBFLog(@"UITableView 的 estimatedRow(SectionHeader / SectionFooter)Height 属性会影响 contentSize、sizeThatFits:、rectForXxx 等方法的计算，导致计算结果不准确，建议重新考虑是否要使用 estimated。可添加 '%@' 的 Symbolic Breakpoint 以捕捉此类信息\n%@", NSStringFromSelector(_cmd), [NSThread callStackSymbols]);
}

- (void)nmui_performBatchUpdates:(void (NS_NOESCAPE ^ _Nullable)(void))updates completion:(void (^ _Nullable)(BOOL finished))completion {
    if (@available(iOS 11.0, *)) {
        ...
        if (!updates && completion) {
            ...
            [self nmbf_performSelector:NSSelectorFromString([NSString stringWithFormat:@"_%@BatchUpdates:%@:", @"perform", @"completion"]) withArguments:&updates, &completion, nil];
        }
    }
}
