...
@implementation UIImageView (NMUI)

NMBFSynthesizeIdStrongProperty(qimgv_animatedImageLayer, setQimgv_animatedImageLayer)
NMBFSynthesizeIdStrongProperty(qimgv_displayLink, setQimgv_displayLink)
NMBFSynthesizeIdStrongProperty(qimgv_animatedImage, setQimgv_animatedImage)
NMBFSynthesizeNSIntegerProperty(qimgv_currentAnimatedImageIndex, setQimgv_currentAnimatedImageIndex)
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        NMBFExtendImplementationOfNonVoidMethodWithSingleArgument([UIImageView class], @selector(initWithImage:), UIImage *, UIImageView *, ^UIImageView *(UIImageView *selfObject, UIImage *image, UIImageView *originReturnValue) {
            [selfObject qimgv_didInitialize];
            return originReturnValue;
        });
        NMBFExtendImplementationOfNonVoidMethodWithTwoArguments([UIImageView class], @selector(initWithImage:highlightedImage:), UIImage *, UIImage *, UIImageView *, ^UIImageView *(UIImageView *selfObject, UIImage *image, UIImage *highlightedImage, UIImageView *originReturnValue) {
            [selfObject qimgv_didInitialize];
            return originReturnValue;
        });
        
        NMBFExtendImplementationOfNonVoidMethodWithSingleArgument([UIImageView class], @selector(initWithFrame:), CGRect, UIImageView *, ^UIImageView *(UIImageView *selfObject, CGRect frame, UIImageView *originReturnValue) {
            [selfObject qimgv_didInitialize];
            return originReturnValue;
        });
        
        NMBFExtendImplementationOfNonVoidMethodWithSingleArgument([UIImageView class], @selector(initWithCoder:), NSCoder *, UIImageView *, ^UIImageView *(UIImageView *selfObject, NSCoder *aDecoder, UIImageView *originReturnValue) {
            [selfObject qimgv_didInitialize];
            return originReturnValue;
        });
        
        NMBFOverrideImplementation([UIImageView class], @selector(setImage:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIImageView *selfObject, UIImage *image) {
                ...
                if (selfObject.nmui_smoothAnimation && image.images) {
        ...
        NMBFOverrideImplementation([UIImageView class], @selector(image), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
        ...
        NMBFExtendImplementationOfVoidMethodWithoutArguments([UIImageView class], @selector(layoutSubviews), ^(UIImageView *selfObject) {
            if (selfObject.qimgv_animatedImageLayer) {
                selfObject.qimgv_animatedImageLayer.frame = selfObject.bounds;
            }
        });
        
        NMBFExtendImplementationOfVoidMethodWithoutArguments([UIImageView class], @selector(didMoveToWindow), ^(UIImageView *selfObject) {
            [selfObject qimgv_updateAnimationStateAutomatically];
        });
        
        NMBFExtendImplementationOfVoidMethodWithSingleArgument([UIImageView class], @selector(setHidden:), BOOL, ^(UIImageView *selfObject, BOOL hidden) {
            [selfObject qimgv_updateAnimationStateAutomatically];
        });
        
        NMBFExtendImplementationOfVoidMethodWithSingleArgument([UIImageView class], @selector(setAlpha:), CGFloat, ^(UIImageView *selfObject, CGFloat alpha) {
            [selfObject qimgv_updateAnimationStateAutomatically];
        });
        
        NMBFExtendImplementationOfVoidMethodWithSingleArgument([UIImageView class], @selector(setFrame:), CGRect, ^(UIImageView *selfObject, CGRect frame) {
            [selfObject qimgv_updateAnimationStateAutomatically];
        });
        
        NMBFOverrideImplementation([UIImageView class], @selector(sizeThatFits:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
...
- (BOOL)qimgv_requestToStartAnimation {
    ...
    self.qimgv_displayLink.paused = self.nmui_pause;
...
- (BOOL)qimgv_canStartAnimation {
    return self.nmui_visible && !CGRectIsEmpty(self.frame);
}
...
- (void)setNmui_smoothAnimation:(BOOL)nmui_smoothAnimation {
    objc_setAssociatedObject(self, &kAssociatedObjectKey_smoothAnimation, @(nmui_smoothAnimation), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (nmui_smoothAnimation && self.image.images && self.image != self.qimgv_animatedImage) {
        self.image = self.image;// 重新设置图片，触发动画
    } else if (!nmui_smoothAnimation && self.qimgv_animatedImage) {
        self.image = self.image;// 交给 setImage 那边把动画清理掉
    }
}

- (BOOL)nmui_smoothAnimation {
...
- (void)setNmui_pause:(BOOL)nmui_pause {
    ...
    if (self.animationImages || self.image.images) {
        self.qimgv_animatedImageLayer.nmui_pause = nmui_pause;
    }
    if (self.qimgv_displayLink) {
        self.qimgv_displayLink.paused = nmui_pause;
    }
}
