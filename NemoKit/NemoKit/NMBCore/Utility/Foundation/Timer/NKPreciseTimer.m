...
static NKPreciseTimer *__sharedInstance = nil;
...
@implementation NKPreciseTimer

+ (void)scheduleAction:(SEL)action target:(id)target inTimeInterval:(NSTimeInterval)timeInterval {
    if ( !__sharedInstance ) __sharedInstance = [[NKPreciseTimer alloc] init];
...
void *thread_entry(void* argument) {
    [(__bridge NKPreciseTimer*)argument thread];
    return NULL;
}
    ...
    while ( 1 ) {
        ...
        @autoreleasepool {
            ...
            if ( (double)(mach_absolute_time() * timebase_ratio) >= time-kSpinLockTime ) {
                ...
                printf("NKPreciseTimer fired: %lfs time discrepancy\n", discrepancy);
#endif
