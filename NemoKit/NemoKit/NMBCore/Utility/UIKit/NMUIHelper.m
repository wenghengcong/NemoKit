...
@implementation NMUIHelper(DynamicType)

+ (NSNumber *)preferredContentSizeLevel {
    NSNumber *index = nil;
    // preferredContentSizeCategory 返回用户选择的字体大小
...
+ (CGFloat)heightForDynamicTypeCell:(NSArray *)heights {
    NSNumber *index = [NMUIHelper preferredContentSizeLevel];
    return [((NSNumber *)[heights objectAtIndex:[index intValue]]) nmbf_CGFloatValue];
}

@end

@implementation NMUIHelper (Keyboard)

/// 键盘是否可见
NMBFSynthesizeBOOLProperty(keyboardVisible, setKeyboardVisible)
/// 键盘高度
NMBFSynthesizeCGFloatProperty(lastKeyboardHeight, setLastKeyboardHeight)

- (void)handleKeyboardWillShow:(NSNotification *)notification {
    self.keyboardVisible = YES;
    self.lastKeyboardHeight = [NMUIHelper keyboardHeightWithNotification:notification];
}
...
+ (BOOL)isKeyboardVisible {
    BOOL visible = [NMUIHelper sharedInstance].keyboardVisible;
    return visible;
}

+ (CGFloat)lastKeyboardHeightInApplicationWindowWhenVisible {
    return [NMUIHelper sharedInstance].lastKeyboardHeight;
}

+ (CGRect)keyboardRectWithNotification:(NSNotification *)notification {
    NSDictionary *userInfo = [notification userInfo];
    CGRect keyboardRect = [[userInfo objectForKey:UIKeyboardFrameEndUserInfoKey] CGRectValue];
    // 注意iOS8以下的系统在横屏时得到的rect，宽度和高度相反了，所以不建议直接通过这个方法获取高度，
    // 而是使用<code>keyboardHeightWithNotification:inView:</code>，因为在后者的实现里会将键盘的rect转换坐标系，
    // 转换过程就会处理横竖屏旋转问题。
    return keyboardRect;
}

+ (CGFloat)keyboardHeightWithNotification:(NSNotification *)notification {
    return [NMUIHelper keyboardHeightWithNotification:notification inView:nil];
}
...
+ (UIViewAnimationOptions)keyboardAnimationOptionsWithNotification:(NSNotification *)notification {
    UIViewAnimationOptions options = [NMUIHelper keyboardAnimationCurveWithNotification:notification]<<16;
    return options;
}
...
NSString *const knmuiResourcesBundleName = @"NMUIResources";

@implementation NMUIHelper (Bundle)

+ (UIImage *)imageWithName:(NSString *)name {
    static NSBundle *resourceBundle = nil;
    if (!resourceBundle) {
        NSBundle *mainBundle = [NSBundle bundleForClass:self];
        NSString *resourcePath = [mainBundle pathForResource:knmuiResourcesBundleName ofType:@"bundle"];
...
@implementation NMUIHelper (UIGraphic)
...
@implementation NMUIHelper (Device)
        ...
        if (isNotchedScreen < 0) {
            if (@available(iOS 12.0, *)) {
                /*
                 ...
                 2. 与触发 [NMUIHelper isNotchedScreen] 方法时的进程有关，例如 https://github.com/Tencent/QMUI_iOS/issues/482#issuecomment-456051738 里提到的 [NSObject performSelectorOnMainThread:withObject:waitUntilDone:NO] 就会导致较多的异常。
                ...
                [[UIScreen mainScreen] nmbf_performSelector:peripheryInsetsSelector withPrimitiveReturnValue:&peripheryInsets];
                ...
                isNotchedScreen = [NMUIHelper is58InchScreen] ? 1 : 0;
            }
        }
    } else {
        isNotchedScreen = 0;
...
+ (BOOL)is65InchScreen {
    if (is65InchScreen < 0) {
        ...
        is65InchScreen = (
                          (DEVICE_WIDTH == self.screenSizeFor65Inch.width && DEVICE_HEIGHT == self.screenSizeFor65Inch.height)
                          && ([[NMUIHelper deviceModel] isEqualToString:@"iPhone11,4"] || [[NMUIHelper deviceModel] isEqualToString:@"iPhone11,6"] || [[NMUIHelper deviceModel] isEqualToString:@"iPhone12,5"])
                          ) ? 1 : 0;
    }
...
+ (BOOL)is61InchScreen {
    if (is61InchScreen < 0) {
        is61InchScreen = (DEVICE_WIDTH == self.screenSizeFor61Inch.width && DEVICE_HEIGHT == self.screenSizeFor61Inch.height && ([[NMUIHelper deviceModel] isEqualToString:@"iPhone11,8"] || [[NMUIHelper deviceModel] isEqualToString:@"iPhone12,1"])) ? 1 : 0;
    }
...
+ (CGFloat)scaleBase375Width:(CGFloat)size {
    return (CGFloat)(size * (SCREEN_WIDTH / 375));
}

+ (CGFloat)scaleBase320Width:(CGFloat)size {
    return (CGFloat)(size * (SCREEN_WIDTH / 320));
}

+ (CGFloat)scaleBase414Width:(CGFloat)size {
    return (CGFloat)(size * (SCREEN_WIDTH / 414));
}
...
+ (CGFloat)preferredLayoutAsSimilarScreenWidthForIPad {
    if (preferredLayoutWidth < 0) {
        ...
        for (NSInteger i = 0; i < widths.count; i++) {
            if (windowWidth <= widths[i].nmbf_CGFloatValue) {
                preferredLayoutWidth = widths[i].nmbf_CGFloatValue;
                continue;
            }
        }
    }
...
+ (BOOL)isHighPerformanceDevice {
    if (isHighPerformanceDevice < 0) {
        NSString *model = [NMUIHelper deviceModel];
        NSString *identifier = [model nmbf_stringMatchedByPattern:@"\\d+"];
...
@implementation NMUIHelper (UIApplication)
...
@implementation NMUIHelper (Animation)
...
+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        [NMUIHelper sharedInstance];// 确保内部的变量、notification 都正确配置
    });
}

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static NMUIHelper *instance = nil;
    dispatch_once(&onceToken,^{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:instance selector:@selector(handleAppSizeWillChange:) name:NMUIAppSizeWillChangeNotification object:nil];
