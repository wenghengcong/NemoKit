...
@implementation UIViewController (NMUIConfiguration)

- (NSArray <UIViewController *>*)nmui_existingViewControllersOfClass:(Class)class {
    NSMutableSet *viewControllers = [NSMutableSet set];
    if (self.presentedViewController) {
        [viewControllers addObjectsFromArray:[self.presentedViewController nmui_existingViewControllersOfClass:class]];
    }
    if ([self isKindOfClass:UINavigationController.class]) {
        [viewControllers addObjectsFromArray:[((UINavigationController *)self).visibleViewController nmui_existingViewControllersOfClass:class]];
    }
    if ([self isKindOfClass:UITabBarController.class]) {
        [viewControllers addObjectsFromArray:[((UITabBarController *)self).selectedViewController nmui_existingViewControllersOfClass:class]];
    }
...
@implementation NMUIConfiguration

+ (instancetype)sharedInstance {
    static dispatch_once_t pred;
    static NMUIConfiguration *sharedInstance;
    dispatch_once(&pred, ^{
        sharedInstance = [[NMUIConfiguration alloc] init];
    });
...
static BOOL NMUI_hasAppliedInitialTemplate;
- (void)applyInitialTemplate {
    if (NMUI_hasAppliedInitialTemplate) {
        return;
    }
    
    // 自动寻找并应用模板
    // Automatically look for templates and apply them
    // @see https://github.com/Tencent/NMUI_iOS/issues/264
    Protocol *protocol = @protocol(NMUIConfigurationTemplateProtocol);
    classref_t *classesref = nil;
    Class *classes = nil;
    int numberOfClasses = nmbf_getProjectClassList(&classesref);
    ...
    for (NSInteger i = 0; i < numberOfClasses; i++) {
        ...
        if ([NSStringFromClass(class) containsString:@"NMUIConfigurationTemplate"] && [class conformsToProtocol:protocol]) {
            if ([class instancesRespondToSelector:@selector(shouldApplyTemplateAutomatically)]) {
                id<NMUIConfigurationTemplateProtocol> template = [[class alloc] init];
                if ([template shouldApplyTemplateAutomatically]) {
                    NMUI_hasAppliedInitialTemplate = YES;
...
#pragma mark - NavigationBar
    ...
    self.navBarCloseButtonImage = [UIImage nmui_imageWithShape:NMUIImageShapeNavClose size:CGSizeMake(16, 16) tintColor:self.navBarTintColor];
    ...
    self.navBarAccessoryViewTypeDisclosureIndicatorImage = [[UIImage nmui_imageWithShape:NMUIImageShapeTriangle size:CGSizeMake(8, 5) tintColor:self.navBarTitleColor] nmui_imageWithOrientation:UIImageOrientationDown];
...
#pragma mark - UIWindowLevel
    self.windowLevelNMUIAlertView = UIWindowLevelAlert - 4.0;
...
#pragma mark - Others
    ...
    self.shouldPrintNMUIWarnLogToConsole = IS_DEBUG;
}
    ...
    if (self.navBarShadowImageColor) {
        ...
        if (shadowImage) {
            if (shadowImage.renderingMode != UIImageRenderingModeAlwaysOriginal) {
                shadowImage = [shadowImage nmui_imageWithTintColor:self.navBarShadowImageColor];
            }
        } else {
            shadowImage = [UIImage nmui_imageWithColor:self.navBarShadowImageColor size:CGSizeMake(4, PixelOne) cornerRadius:0];
        }
    }
...
- (void)setNavBarBackIndicatorImage:(UIImage *)navBarBackIndicatorImage {
    ...
    if (navBarBackIndicatorImage && self.sizeNavBarBackIndicatorImageAutomatically) {
        ...
        if (!CGSizeEqualToSize(customBackIndicatorImageSize, systemBackIndicatorImageSize)) {
            ...
            _navBarBackIndicatorImage = [[_navBarBackIndicatorImage nmui_imageWithSpacingExtensionInsets:UIEdgeInsetsMake(imageExtensionVerticalFloat,
                                                                                                                          0,
...
- (void)setToolBarShadowImageColor:(UIColor *)toolBarShadowImageColor {
    ...
    UIImage *shadowImage = toolBarShadowImageColor ? [UIImage nmui_imageWithColor:_toolBarShadowImageColor size:CGSizeMake(1, PixelOne) cornerRadius:0] : nil;
...
#endif
        UIImage *shadowImage = [UIImage nmui_imageWithColor:_tabBarShadowImageColor size:CGSizeMake(1, PixelOne) cornerRadius:0];
...
#ifdef IOS13_SDK_ALLOWED
    if (@available(iOS 13.0, *)) {
        [self.tabBarAppearance nmui_applyItemAppearanceWithBlock:^(UITabBarItemAppearance * _Nonnull itemAppearance) {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        [self.appearanceUpdatingTabBarControllers enumerateObjectsUsingBlock:^(UITabBarController * _Nonnull tabBarController, NSUInteger idx, BOOL * _Nonnull stop) {
            [tabBarController.tabBar.items enumerateObjectsUsingBlock:^(UITabBarItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                [item nmui_updateTintColorForiOS12AndEarlier:tabBarItemImageColor];
...
- (void)setStatusbarStyleLightInitially:(BOOL)statusbarStyleLightInitially {
    ...
    [[NMUIHelper visibleViewController] setNeedsStatusBarAppearanceUpdate];
}

#pragma mark - Appearance Updating Views

// 解决某些场景下更新配置表无法覆盖样式的问题 https://github.com/Tencent/NMUI_iOS/issues/700
...
- (NSArray <UIViewController *>*)appearanceUpdatingViewControllersOfClass:(Class)class {
    ...
    [UIApplication.sharedApplication.windows enumerateObjectsUsingBlock:^(__kindof UIWindow * _Nonnull window, NSUInteger idx, BOOL * _Nonnull stop) {
        if (window.rootViewController) {
            [viewControllers addObjectsFromArray:[window.rootViewController nmui_existingViewControllersOfClass:class]];
        }
    }];
...
const NSInteger NMUIImageOriginalRenderingModeDefault = -1;

@interface UIImage (NMUIConfiguration)

@property(nonatomic, assign) UIImageRenderingMode nmui_originalRenderingMode;
@property(nonatomic, assign) BOOL qimgconf_hasSetOriginalRenderingMode;
@end

@implementation UITabBarItem (NMUIConfiguration)
...
- (void)nmui_updateTintColorForiOS12AndEarlier:(UIColor *)tintColor {
    if (@available(iOS 13.0, *)) return;
    if (!tintColor) return;
    
    UIImage *image = self.image;
    UIImageRenderingMode renderingMode = image.renderingMode;
    UIImageRenderingMode originalRenderingMode = image.nmui_originalRenderingMode;
    if (originalRenderingMode == NMUIImageOriginalRenderingModeDefault) {
        if (renderingMode != UIImageRenderingModeAlwaysOriginal) {
            image = [[image nmui_imageWithTintColor:TabBarItemImageColor] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
            image.nmui_originalRenderingMode = renderingMode;
            self.image = image;
        }
    } else if (originalRenderingMode != UIImageRenderingModeAlwaysOriginal && renderingMode == UIImageRenderingModeAlwaysOriginal) {
        image = [[image nmui_imageWithTintColor:TabBarItemImageColor] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        image.nmui_originalRenderingMode = originalRenderingMode;
        self.image = image;
    }
}
...
@implementation UIImage (NMUIConfiguration)

NMBFSynthesizeBOOLProperty(qimgconf_hasSetOriginalRenderingMode, setQimgconf_hasSetOriginalRenderingMode)

static char kAssociatedObjectKey_originalRenderingMode;
- (void)setNmui_originalRenderingMode:(UIImageRenderingMode)nmui_originalRenderingMode {
    self.qimgconf_hasSetOriginalRenderingMode = YES;
        objc_setAssociatedObject(self, &kAssociatedObjectKey_originalRenderingMode, @(nmui_originalRenderingMode), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }

    - (UIImageRenderingMode)nmui_originalRenderingMode {
        if (!self.qimgconf_hasSetOriginalRenderingMode) return NMUIImageOriginalRenderingModeDefault;
