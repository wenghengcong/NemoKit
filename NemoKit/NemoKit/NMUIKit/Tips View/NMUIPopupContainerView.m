...
@interface NMUIPopupContainerViewWindow : UIWindow

@end

@interface NMUIPopContainerViewController : NMUICommonViewController

@end

@interface NMUIPopContainerMaskControl : UIControl

@property(nonatomic, weak) NMUIPopupContainerView *popupContainerView;
@end

@interface NMUIPopupContainerView (UIAppearance)

- (void)updateAppearance;
@end

@interface NMUIPopupContainerView () {
    UIImageView                     *_imageView;
    UILabel                         *_textLabel;
}

@property(nonatomic, strong) NMUIPopupContainerViewWindow *popupWindow;
@property(nonatomic, weak) UIWindow *previousKeyWindow;
@property(nonatomic, assign) BOOL hidesByUserTap;
@end

@implementation NMUIPopupContainerView
...
- (void)dealloc {
    _sourceView.nmui_frameDidChangeBlock = nil;
}
...
- (void)layoutSubviews {
    ...
    CGRect roundedRect = CGRectMake(self.borderWidth / 2.0, self.borderWidth / 2.0 + (self.currentLayoutDirection == NMUIPopupContainerViewLayoutDirectionAbove ? 0 : arrowSize.height), CGRectGetWidth(self.bounds) - self.borderWidth, CGRectGetHeight(self.bounds) - arrowSize.height - self.borderWidth);
    ...
    if (self.currentLayoutDirection == NMUIPopupContainerViewLayoutDirectionAbove) {
        // 让开，我要开始开始画三角形了，箭头向下
    ...
    if (self.currentLayoutDirection == NMUIPopupContainerViewLayoutDirectionBelow) {
        // 箭头向上
...
- (void)setSourceBarItem:(__kindof UIBarItem *)sourceBarItem {
    ...
    sourceBarItem.nmui_viewLayoutDidChangeBlock = ^(__kindof UIBarItem * _Nonnull item, UIView * _Nullable view) {
        ...
        CGRect rect = [view nmui_convertRect:view.bounds toView:convertToView];
        weakSelf.sourceRect = rect;
    };

    if (sourceBarItem.nmui_view && sourceBarItem.nmui_viewLayoutDidChangeBlock) {
        sourceBarItem.nmui_viewLayoutDidChangeBlock(sourceBarItem, sourceBarItem.nmui_view);// update layout immediately
    }
}
...
- (void)setSourceView:(__kindof UIView *)sourceView {
    ...
    __weak __typeof(self)weakSelf = self;
    sourceView.nmui_frameDidChangeBlock = ^(__kindof UIView * _Nonnull view, CGRect precedingFrame) {
        if (!view.window || !weakSelf.superview) return;
        UIView *convertToView = weakSelf.popupWindow ? UIApplication.sharedApplication.delegate.window : weakSelf.superview;// 对于以 window 方式显示的情况，由于横竖屏旋转时，不同 window 的旋转顺序不同，所以可能导致 sourceBarItem 所在的 window 已经旋转了但 popupWindow 还没旋转（iOS 11 及以后），那么计算出来的坐标就错了，所以这里改为用 UIApplication window
        CGRect rect = [view nmui_convertRect:view.bounds toView:convertToView];
        weakSelf.sourceRect = rect;
    };
    sourceView.nmui_frameDidChangeBlock(sourceView, sourceView.frame);// update layout immediately
}
...
- (void)layoutWithTargetRect:(CGRect)targetRect {
    ...
    BOOL canShowAtAbove = [self canTipShowAtSpecifiedLayoutDirect:NMUIPopupContainerViewLayoutDirectionAbove targetRect:targetRect tipSize:tipSize];
    BOOL canShowAtBelow = [self canTipShowAtSpecifiedLayoutDirect:NMUIPopupContainerViewLayoutDirectionBelow targetRect:targetRect tipSize:tipSize];
    ...
    if (!canShowAtAbove && !canShowAtBelow) {
        ...
        _currentLayoutDirection = maximumHeightAbove > maximumHeightBelow ? NMUIPopupContainerViewLayoutDirectionAbove : NMUIPopupContainerViewLayoutDirectionBelow;
        
        NMBFLog(NSStringFromClass(self.class), @"%@, 因为上下都不够空间，所以最大高度被强制改为%@, 位于目标的%@", self, @(self.maximumHeight), maximumHeightAbove > maximumHeightBelow ? @"上方" : @"下方");
        
    } else if (_currentLayoutDirection == NMUIPopupContainerViewLayoutDirectionAbove && !canShowAtAbove) {
        _currentLayoutDirection = NMUIPopupContainerViewLayoutDirectionBelow;
    } else if (_currentLayoutDirection == NMUIPopupContainerViewLayoutDirectionBelow && !canShowAtBelow) {
        _currentLayoutDirection = NMUIPopupContainerViewLayoutDirectionAbove;
    }
    ...
    if (_currentLayoutDirection == NMUIPopupContainerViewLayoutDirectionAbove) {
    ...
    } else if (_currentLayoutDirection == NMUIPopupContainerViewLayoutDirectionBelow) {
...
- (CGFloat)tipMinYWithTargetRect:(CGRect)itemRect tipSize:(CGSize)tipSize preferLayoutDirection:(NMUIPopupContainerViewLayoutDirection)direction {
    CGFloat tipMinY = 0;
    if (direction == NMUIPopupContainerViewLayoutDirectionAbove) {
        tipMinY = CGRectGetMinY(itemRect) - tipSize.height - self.distanceBetweenSource;
    } else if (direction == NMUIPopupContainerViewLayoutDirectionBelow) {
        tipMinY = CGRectGetMaxY(itemRect) + self.distanceBetweenSource;
    }
    return tipMinY;
}

- (BOOL)canTipShowAtSpecifiedLayoutDirect:(NMUIPopupContainerViewLayoutDirection)direction targetRect:(CGRect)itemRect tipSize:(CGSize)tipSize {
    BOOL canShow = NO;
...
- (void)showWithAnimated:(BOOL)animated completion:(void (^)(BOOL))completion {
    ...
    if (!self.superview) {
        ...
        NMUICommonViewController *viewController = (NMUICommonViewController *)self.popupWindow.rootViewController;
        viewController.supportedOrientationMask = [NMUIHelper visibleViewController].supportedInterfaceOrientations;
...
- (void)initPopupContainerViewWindowIfNeeded {
    if (!self.popupWindow) {
        self.popupWindow = [[NMUIPopupContainerViewWindow alloc] init];
        self.popupWindow.nmui_capturesStatusBarAppearance = NO;
        self.popupWindow.backgroundColor = UIColorClear;
        self.popupWindow.windowLevel = UIWindowLevelNMUIAlertView;
        NMUIPopContainerViewController *viewController = [[NMUIPopContainerViewController alloc] init];
        ((NMUIPopContainerMaskControl *)viewController.view).popupContainerView = self;
...
@implementation NMUIPopupContainerView (UISubclassingHooks)
...
@implementation NMUIPopupContainerView (UIAppearance)
...
+ (void)setDefaultAppearance {
    NMUIPopupContainerView *appearance = [NMUIPopupContainerView appearance];
    ...
    appearance.preferLayoutDirection = NMUIPopupContainerViewLayoutDirectionAbove;
    ...
    appearance.nmui_outsideEdge = UIEdgeInsetsZero;
...
- (void)updateAppearance {
    ...
    self.nmui_outsideEdge = appearance.nmui_outsideEdge;
}

@end

@implementation NMUIPopContainerViewController

- (void)loadView {
    NMUIPopContainerMaskControl *maskControl = [[NMUIPopContainerMaskControl alloc] init];
    self.view = maskControl;
}

@end

@implementation NMUIPopContainerMaskControl
...
@implementation NMUIPopupContainerViewWindow
