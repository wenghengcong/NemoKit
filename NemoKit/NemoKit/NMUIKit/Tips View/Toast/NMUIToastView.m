...
static NSMutableArray <NMUIToastView *> *kToastViews = nil;

@interface NMUIToastView ()

@property(nonatomic, weak) NSTimer *hideDelayTimer;

@end

@implementation NMUIToastView
...
- (void)didInitialize {
    ...
    self.toastPosition = NMUIToastViewPositionCenter;
...
- (NMUIToastAnimator *)defaultAnimator {
    NMUIToastAnimator *toastAnimator = [[NMUIToastAnimator alloc] initWithToastView:self];
    return toastAnimator;
}

- (UIView *)defaultBackgrondView {
    NMUIToastBackgroundView *backgroundView = [[NMUIToastBackgroundView alloc] init];
    return backgroundView;
}

- (UIView *)defaultContentView {
    NMUIToastContentView *contentView = [[NMUIToastContentView alloc] init];
    return contentView;
}
...
- (void)layoutSubviews {
    ...
    UIEdgeInsets marginInsets = UIEdgeInsetsConcat(self.marginInsets, self.parentView.nmui_safeAreaInsets);
    ...
    if ([NMUIKeyboardManager isKeyboardVisible]) {
        ...
        CGRect keyboardFrame = [NMUIKeyboardManager currentKeyboardFrame];
        CGRect parentViewRect = [[NMUIKeyboardManager keyboardWindow] convertRect:self.parentView.frame fromView:self.parentView.superview];
    ...
    if (self.contentView) {
        ...
        if (self.toastPosition == NMUIToastViewPositionTop) {
            contentViewY = marginInsets.top + self.offset.y;
        } else if (self.toastPosition == NMUIToastViewPositionBottom) {
        ...
        self.contentView.nmui_frameApplyTransform = contentRect;
    }
    if (self.backgroundView) {
...
@interface NMUIToastView (UIAppearance)

@end

@implementation NMUIToastView (UIAppearance)
...
+ (void)setDefaultAppearance {
    NMUIToastView *appearance = [NMUIToastView appearance];
