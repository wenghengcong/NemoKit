...
@implementation NMUIToastContentView
...
- (void)layoutSubviews {
    ...
    CGFloat contentLimitWidth = self.nmui_width - UIEdgeInsetsGetHorizontalValue(self.insets);
    CGSize contentSize = [self sizeThatFits:self.bounds.size shouldConsiderMinimumSize:NO];
    CGFloat minY = self.insets.top + CGFloatGetCenter(self.nmui_height - UIEdgeInsetsGetVerticalValue(self.insets), contentSize.height - UIEdgeInsetsGetVerticalValue(self.insets));
    if (hasCustomView) {
        self.customView.nmui_left = self.insets.left + CGFloatGetCenter(contentLimitWidth, self.customView.nmui_width);
        self.customView.nmui_top = minY;
        minY = self.customView.nmui_bottom + self.customViewMarginBottom;
    }
    if (hasTextLabel) {
        CGSize textLabelSize = [self.textLabel sizeThatFits:CGSizeMake(contentLimitWidth, CGFLOAT_MAX)];
        self.textLabel.nmui_left = self.insets.left;
        self.textLabel.nmui_top = minY;
        self.textLabel.nmui_width = contentLimitWidth;
        self.textLabel.nmui_height = textLabelSize.height;
        minY = self.textLabel.nmui_bottom + self.textLabelMarginBottom;
    }
    if (hasDetailTextLabel) {
        CGSize detailTextLabelSize = [self.detailTextLabel sizeThatFits:CGSizeMake(contentLimitWidth, CGFLOAT_MAX)];
        self.detailTextLabel.nmui_left = self.insets.left;
        self.detailTextLabel.nmui_top = minY;
        self.detailTextLabel.nmui_width = contentLimitWidth;
        self.detailTextLabel.nmui_height = detailTextLabelSize.height;
    }
}
...
@interface NMUIToastContentView (UIAppearance)

@end

@implementation NMUIToastContentView (UIAppearance)
...
+ (void)setDefaultAppearance {
    NMUIToastContentView *appearance = [NMUIToastContentView appearance];
    ...
    appearance.textLabelAttributes = @{NSFontAttributeName: UIFontBoldMake(16), NSParagraphStyleAttributeName: [NSMutableParagraphStyle nmui_paragraphStyleWithLineHeight:22 lineBreakMode:NSLineBreakByWordWrapping textAlignment:NSTextAlignmentCenter]};
    appearance.detailTextLabelAttributes = @{NSFontAttributeName: UIFontBoldMake(12), NSParagraphStyleAttributeName: [NSMutableParagraphStyle nmui_paragraphStyleWithLineHeight:18 lineBreakMode:NSLineBreakByWordWrapping textAlignment:NSTextAlignmentCenter]};
}
