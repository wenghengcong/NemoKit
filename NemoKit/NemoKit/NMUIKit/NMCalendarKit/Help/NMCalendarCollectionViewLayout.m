//
//  NMCalendarCollectionViewLayout.m
//  Nemo
//
...
@interface NMCalendarCollectionViewLayout ()
...
@implementation NMCalendarCollectionViewLayout
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [self registerClass:NMCalendarSeparatorDecorationView.class forDecorationViewOfKind:kFSCalendarSeparatorInterRows];
    }
    return self;
}
...
- (void)prepareLayout
{
    ...
    self.headerReferenceSize = ({
        ...
        if (self.calendar.floatingMode) {
            ...
            headerSize = CGSizeMake(self.collectionView.nmui_width, headerHeight);
        }
        headerSize;
    });
    self.estimatedItemSize = ({
        CGFloat width = (self.collectionView.nmui_width-self.sectionInsets.left-self.sectionInsets.right)/7.0;
        CGFloat height = ({
            CGFloat height = NMCalendarStandardRowHeight;
            if (!self.calendar.floatingMode) {
                switch (self.calendar.transitionCoordinator.representingScope) {
                    case NMCalendarScopeMonth: {
                        height = (self.collectionView.nmui_height-self.sectionInsets.top-self.sectionInsets.bottom)/6.0;
                        break;
                    }
                    case NMCalendarScopeWeek: {
                        height = (self.collectionView.nmui_height-self.sectionInsets.top-self.sectionInsets.bottom);
                        break;
                    }
                    default:
                        break;
    ...
    self.widths = ({
        ...
        CGFloat contentWidth = self.collectionView.nmui_width - self.sectionInsets.left - self.sectionInsets.right;
        NMCalendarSliceCake(contentWidth, columnCount, widths);
        widths;
    });
    ...
    free(self.heights);
    self.heights = ({
        NSInteger rowCount = self.calendar.transitionCoordinator.representingScope == NMCalendarScopeWeek ? 1 : 6;
        size_t rowSize = sizeof(CGFloat)*rowCount;
        CGFloat *heights = malloc(rowSize);
        if (!self.calendar.floatingMode) {
            CGFloat contentHeight = self.collectionView.nmui_height - self.sectionInsets.top - self.sectionInsets.bottom;
            NMCalendarSliceCake(contentHeight, rowCount, heights);
        } else {
    ...
    self.contentSize = ({
        ...
        if (!self.calendar.floatingMode) {
            CGFloat width = self.collectionView.nmui_width;
            CGFloat height = self.collectionView.nmui_height;
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    if (!self.calendar.floatingMode) {
        ...
        switch (self.scrollDirection) {
            case UICollectionViewScrollDirectionHorizontal: {
                ...
                NSInteger startColumn = ({
                    NSInteger startSection = rect.origin.x/self.collectionView.nmui_width;
                    CGFloat widthDelta = NMBMathMod(CGRectGetMinX(rect), self.collectionView.nmui_width)-self.sectionInsets.left;
                    widthDelta = MIN(MAX(0, widthDelta), self.collectionView.nmui_width-self.sectionInsets.left);
                    NSInteger countDelta = NMBMathFloor(widthDelta/self.estimatedItemSize.width);
                ...
                NSInteger endColumn = ({
                    NSInteger endColumn;
                    CGFloat section = CGRectGetMaxX(rect)/self.collectionView.nmui_width;
                    CGFloat remainder = NMBMathMod(section, 1);
                    // https://stackoverflow.com/a/10335601/2398107
                    if (remainder <= MAX(100*FLT_EPSILON*ABS(remainder), FLT_MIN)) {
                        endColumn = NMBMathFloor(section)*7 - 1;
                    } else {
                        CGFloat widthDelta = NMBMathMod(CGRectGetMaxX(rect), self.collectionView.nmui_width)-self.sectionInsets.left;
                        widthDelta = MIN(MAX(0, widthDelta), self.collectionView.nmui_width - self.sectionInsets.left);
                        NSInteger countDelta = NMBMathCeil(widthDelta/self.estimatedItemSize.width);
                        endColumn = NMBMathFloor(section)*7 + countDelta - 1;
                    }
                    endColumn;
                });
                
                NSInteger numberOfRows = self.calendar.transitionCoordinator.representingScope == NMCalendarScopeMonth ? 6 : 1;
            ...
            case UICollectionViewScrollDirectionVertical: {
                ...
                NSInteger startRow = ({
                    NSInteger startSection = rect.origin.y/self.collectionView.nmui_height;
                    CGFloat heightDelta = NMBMathMod(CGRectGetMinY(rect), self.collectionView.nmui_height)-self.sectionInsets.top;
                    heightDelta = MIN(MAX(0, heightDelta), self.collectionView.nmui_height-self.sectionInsets.top);
                    NSInteger countDelta = NMBMathFloor(heightDelta/self.estimatedItemSize.height);
                ...
                NSInteger endRow = ({
                    NSInteger endRow;
                    CGFloat section = CGRectGetMaxY(rect)/self.collectionView.nmui_height;
                    CGFloat remainder = NMBMathMod(section, 1);
                    // https://stackoverflow.com/a/10335601/2398107
                    if (remainder <= MAX(100*FLT_EPSILON*ABS(remainder), FLT_MIN)) {
                        endRow = NMBMathFloor(section)*6 - 1;
                    } else {
                        CGFloat heightDelta = NMBMathMod(CGRectGetMaxY(rect), self.collectionView.nmui_height)-self.sectionInsets.top;
                        heightDelta = MIN(MAX(0, heightDelta), self.collectionView.nmui_height-self.sectionInsets.top);
                        NSInteger countDelta = NMBMathCeil(heightDelta/self.estimatedItemSize.height);
                        endRow = NMBMathFloor(section)*6 + countDelta-1;
                    }
                    endRow;
                });
        ...
        NSInteger startRowIndex = ({
            ...
            NSInteger startRowCount = NMBMathCeil(heightDelta1/self.estimatedItemSize.height);
        ...
        NSInteger endRowIndex = ({
            ...
            NSInteger endRowCount = NMBMathCeil(heightDelta2/self.estimatedItemSize.height);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!attributes) {
        ...
        CGRect frame = ({
            ...
            switch (self.scrollDirection) {
                case UICollectionViewScrollDirectionHorizontal: {
                    x = self.lefts[column] + indexPath.section * self.collectionView.nmui_width;
                ...
                case UICollectionViewScrollDirectionVertical: {
                    x = self.lefts[column];
                    if (!self.calendar.floatingMode) {
                        CGFloat sectionTop = indexPath.section * self.collectionView.nmui_height;
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath
{
    if ([elementKind isEqualToString:UICollectionElementKindSectionHeader]) {
        ...
        if (!attributes) {
            ...
            attributes.frame = CGRectMake(0, self.sectionTops[indexPath.section], self.collectionView.nmui_width, self.headerReferenceSize.height);
...
{
    if ([elementKind isEqualToString:kFSCalendarSeparatorInterRows] && (self.separators & NMCalendarSeparatorInterRows)) {
        ...
        if (!attributes) {
            ...
            if (!self.calendar.floatingMode) {
                ...
                switch (self.scrollDirection) {
                    case UICollectionViewScrollDirectionHorizontal: {
                        x = self.lefts[coordinate.column] + indexPath.section * self.collectionView.nmui_width;
                        y = rowOffset;
                        break;
                    }
                    case UICollectionViewScrollDirectionVertical: {
                        x = 0;
                        y = indexPath.section * self.collectionView.nmui_height + rowOffset;
                        break;
                    }
                    default:
                        break;
            ...
            CGFloat width = self.collectionView.nmui_width;
            CGFloat height = NMCalendarStandardSeparatorThickness;
