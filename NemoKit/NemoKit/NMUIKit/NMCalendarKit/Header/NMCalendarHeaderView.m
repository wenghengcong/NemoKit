...
@interface NMCalendarHeaderView ()<UICollectionViewDataSource,UICollectionViewDelegate, NMCalendarCollectionViewInternalDelegate>

- (void)scrollToOffset:(CGFloat)scrollOffset animated:(BOOL)animated;
- (void)configureCell:(NMCalendarHeaderCell *)cell atIndexPath:(NSIndexPath *)indexPath;

@end

@implementation NMCalendarHeaderView
...
- (void)initialize
{
    ...
    NMCalendarHeaderLayout *collectionViewLayout = [[NMCalendarHeaderLayout alloc] init];
    self.collectionViewLayout = collectionViewLayout;
    
    NMCalendarCollectionView *collectionView = [[NMCalendarCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:collectionViewLayout];
    ...
    [collectionView registerClass:[NMCalendarHeaderCell class] forCellWithReuseIdentifier:@"cell"];
...
- (void)layoutSubviews
{
    ...
    self.collectionView.frame = CGRectMake(0, self.nmui_height*0.1, self.nmui_width, self.nmui_height*0.9);
}
...
{
    NMCalendarHeaderCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
    cell.header = self;
...
- (void)scrollToOffset:(CGFloat)scrollOffset animated:(BOOL)animated
{
    if (self.scrollDirection == UICollectionViewScrollDirectionHorizontal) {
        CGFloat step = self.collectionView.nmui_width*((self.scrollDirection==UICollectionViewScrollDirectionHorizontal)?0.5:1);
        [_collectionView setContentOffset:CGPointMake((scrollOffset+0.5)*step, 0) animated:animated];
    } else {
        CGFloat step = self.collectionView.nmui_height;
...
- (void)configureCell:(NMCalendarHeaderCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    NMCalendarAppearance *appearance = self.calendar.appearance;
    ...
    BOOL usesUpperCase = (appearance.caseOptions & 15) == NMCalendarCaseOptionsHeaderUsesUpperCase;
    NSString *text = nil;
    switch (self.calendar.transitionCoordinator.representingScope) {
        ...
        case NMCalendarScopeWeek: {
            if ((indexPath.item == 0 || indexPath.item == [self.collectionView numberOfItemsInSection:0] - 1)) {
                ...
                NSDate *firstPage = [self.calendar.gregorian nm_middleDayOfWeek:self.calendar.minimumDate];
...
{
    [self.collectionView.visibleCells enumerateObjectsUsingBlock:^(__kindof NMCalendarHeaderCell * _Nonnull cell, NSUInteger idx, BOOL * _Nonnull stop) {
...
@implementation NMCalendarHeaderCell
...
- (void)layoutSubviews
{
    ...
    if (self.header.scrollDirection == UICollectionViewScrollDirectionHorizontal) {
        ...
        if (self.header.scrollEnabled) {
            self.contentView.alpha = 1.0 - (1.0-self.header.calendar.appearance.headerMinimumDissolvedAlpha)*ABS(center-position)/self.nmui_width;
        } else {
            self.contentView.alpha = (position > self.header.nmui_width*0.25 && position < self.header.nmui_width*0.75);
        }
        ...
        self.contentView.alpha = 1.0 - (1.0-self.header.calendar.appearance.headerMinimumDissolvedAlpha)*ABS(center-position)/self.nmui_height;
    }
}
...
@implementation NMCalendarHeaderLayout
...
- (void)prepareLayout
{
    ...
    self.itemSize = CGSizeMake(
                               self.collectionView.nmui_width*((self.scrollDirection==UICollectionViewScrollDirectionHorizontal)?0.5:1),
                               self.collectionView.nmui_height
                              );
...
@implementation NMCalendarHeaderTouchDeliver
