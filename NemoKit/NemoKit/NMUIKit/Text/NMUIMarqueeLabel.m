...
@implementation NMUIMarqueeLabel
...
- (void)checkIfShouldShowGradientLayer {
    ...
    if (shouldShowFadeLayer) {
        ...
        [self setNeedsLayout];// fadeLayer 作为 layer.mask，它依赖于在 layoutSubviews 里正确布局，否则会因为错误的 size 而导致 label 看不见o
    } else {
...
@implementation NMUIMarqueeLabel (ReusableView)
