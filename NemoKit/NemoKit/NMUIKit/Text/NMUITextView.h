...
@protocol NMUITextViewDelegate <UITextViewDelegate>
 ...
 */
- (void)textView:(NMUITextView *)textView newHeightAfterTextChanged:(CGFloat)height;
 ...
 */
- (BOOL)textViewShouldReturn:(NMUITextView *)textView;
 ...
 */
- (void)textView:(NMUITextView *)textView didPreventTextChangeInRange:(NSRange)range replacementText:(NSString *)replacementText;
 ...
 */
@interface NMUITextView : UITextView

@property(nonatomic, weak) id<NMUITextViewDelegate> delegate;
...
/**
 *  在使用 maximumTextLength 功能的时候，是否应该把文字长度按照 [NSString (NMUI) nmbf_lengthWhenCountingNonASCIICharacterAsTwo] 的方法来计算。
 *  默认为 NO。
 */
