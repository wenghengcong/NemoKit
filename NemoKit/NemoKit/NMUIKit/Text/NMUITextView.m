...
// 私有的类，专用于实现 NMUITextViewDelegate，避免 self.delegate = self 的写法（以前是 NMUITextView 自己实现了 delegate）
@interface _NMUITextViewDelegator : NSObject <NMUITextViewDelegate>

@property(nonatomic, weak) NMUITextView *textView;
@end

@interface NMUITextView ()

@property(nonatomic, assign) BOOL debug;
@property(nonatomic, assign) BOOL postInitializationMethodCalled;
@property(nonatomic, strong) _NMUITextViewDelegator *delegator;
...
- (void)didInitialize {
    ...
    self.delegator = [[_NMUITextViewDelegator alloc] init];
...
- (void)handleTextChanged:(id)sender {
    ...
    NMUITextView *textView = nil;
    
    if ([sender isKindOfClass:[NSNotification class]]) {
        id object = ((NSNotification *)sender).object;
        if (object == self) {
            textView = (NMUITextView *)object;
        }
    } else if ([sender isKindOfClass:[NMUITextView class]]) {
        textView = (NMUITextView *)sender;
    }
    ...
    if (textView) {
        ...
        if ([textView.delegate respondsToSelector:@selector(textView:newHeightAfterTextChanged:)]) {
            ...
            if (textView.debug) NMBFLog(NSStringFromClass(textView.class), @"handleTextDidChange, text = %@, resultHeight = %f", textView.text, resultHeight);
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [textView nmui_scrollCaretVisibleAnimated:NO];
        });
    }
}
...
- (void)setContentOffset:(CGPoint)contentOffset animated:(BOOL)animated {
    if (!self.shouldRejectSystemScroll) {
        ...
        if (self.debug) NMBFLog(NSStringFromClass(self.class), @"%@, contentOffset.y = %.2f", NSStringFromSelector(_cmd), contentOffset.y);
    } else {
        if (self.debug) NMBFLog(NSStringFromClass(self.class), @"被屏蔽的 %@, contentOffset.y = %.2f", NSStringFromSelector(_cmd), contentOffset.y);
    }
}
...
@implementation _NMUITextViewDelegator

#pragma mark - <NMUITextViewDelegate>

- (BOOL)textView:(NMUITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if (self.textView.debug) NMBFLog(NSStringFromClass(self.class), @"textView.text(%@ | %@) = %@\nmarkedTextRange = %@\nrange = %@\ntext = %@", @(textView.text.length), @(textView.text.nmbf_lengthWhenCountingNonASCIICharacterAsTwo), textView.text, textView.markedTextRange, NSStringFromRange(range), text);
    ...
    if (textView.maximumTextLength < NSUIntegerMax) {
        ...
        NSUInteger rangeLength = textView.shouldCountingNonASCIICharacterAsTwo ? [textView.text substringWithRange:range].nmbf_lengthWhenCountingNonASCIICharacterAsTwo : range.length;
        ...
        if (textWillOutofMaximumTextLength) {
            ...
            if (substringLength > 0 && [textView lengthWithString:text] > substringLength) {
                NSString *allowedText = [text nmbf_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, substringLength) lessValue:YES countingNonASCIICharacterAsTwo:textView.shouldCountingNonASCIICharacterAsTwo];
...
- (void)textViewDidChange:(NMUITextView *)textView {
    ...
    if (!textView.markedTextRange) {
        if ([textView lengthWithString:textView.text] > textView.maximumTextLength) {
            ...
            textView.text = [textView.text nmbf_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, textView.maximumTextLength) lessValue:YES countingNonASCIICharacterAsTwo:textView.shouldCountingNonASCIICharacterAsTwo];
