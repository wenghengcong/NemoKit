//
//  NMUIEasings.h
//  Nemo
//
//  Created by Hunt on 2019/11/3.
//  Copyright © 2019 LuCi. All rights reserved.
//

#import <UIKit/UIKit.h>

/// https://easings.net
/// http://cubic-bezier.com

CG_INLINE CGFloat
NMUI_Linear(CGFloat t) {
    return t;
}

CG_INLINE CGFloat
NMUI_EaseInSine(CGFloat t) {
    return 1 - cos(t * M_PI_2);
}

CG_INLINE CGFloat
NMUI_EaseOutSine(CGFloat t) {
    return sin(t * M_PI_2);
}

CG_INLINE CGFloat
NMUI_EaseInOutSine(CGFloat t) {
    return - (cos(M_PI * t) - 1) / 2;
}

CG_INLINE CGFloat
NMUI_EaseInQuad(CGFloat t) {
    return pow(t, 2);
}

CG_INLINE CGFloat
NMUI_EaseOutQuad(CGFloat t) {
    return 1 - pow(1 - t, 2);
}

CG_INLINE CGFloat
NMUI_EaseInOutQuad(CGFloat t) {
    return t < 0.5 ? (2 * pow(t, 2)) : (1 - pow(-2 * t + 2, 2) / 2);
}

CG_INLINE CGFloat
NMUI_EaseInCubic(CGFloat t) {
    return pow(t, 3);
}

CG_INLINE CGFloat
NMUI_EaseOutCubic(CGFloat t) {
    return 1 - pow(1 - t, 3);
}

CG_INLINE CGFloat
NMUI_EaseInOutCubic(CGFloat t) {
    return t < 0.5 ? (4 * pow(t, 3)) : (1 - pow(-2 * t + 2, 3) / 2);
}

CG_INLINE CGFloat
NMUI_EaseInQuart(CGFloat t) {
    return pow(t, 4);
}

CG_INLINE CGFloat
NMUI_EaseOutQuart(CGFloat t) {
    return 1 - pow(1 - t, 4);
}

CG_INLINE CGFloat
NMUI_EaseInOutQuart(CGFloat t) {
    return t < 0.5 ? (8 * pow(t, 4)) : (1 - pow(-2 * t + 2, 4) / 2);
}

CG_INLINE CGFloat
NMUI_EaseInQuint(CGFloat t) {
    return pow(t, 5);
}

CG_INLINE CGFloat
NMUI_EaseOutQuint(CGFloat t) {
    return 1 - pow(1 - t, 5);
}

CG_INLINE CGFloat
NMUI_EaseInOutQuint(CGFloat t) {
    return t < 0.5 ? (16 * pow(t, 5)) : (1 - pow(-2 * t + 2, 5) / 2);
}

CG_INLINE CGFloat
NMUI_EaseInExpo(CGFloat t) {
    return t == 0 ? 0 : pow(2, 10 * t - 10);
}

CG_INLINE CGFloat
NMUI_EaseOutExpo(CGFloat t) {
    return t == 1 ? 1 : 1 - pow(2, -10 * t);
}

CG_INLINE CGFloat
NMUI_EaseInOutExpo(CGFloat t) {
    return t == 0 ? 0 : t == 1 ? 1 : t < 0.5 ? pow(2, 20 * t - 10 ) / 2 : (2 - pow(2, -20 * t + 10 )) / 2;
}

CG_INLINE CGFloat
NMUI_EaseInCirc(CGFloat t) {
    return 1 - sqrt(1 - pow(t, 2));
}

CG_INLINE CGFloat
NMUI_EaseOutCirc(CGFloat t) {
    return sqrt(1 - pow(t - 1, 2));
}

CG_INLINE CGFloat
NMUI_EaseInOutCirc(CGFloat t) {
    return t < 0.5 ? (1 - sqrt(1 - pow(2 * t, 2))) / 2 : (sqrt(1 - pow(-2 * t + 2, 2)) + 1) / 2;
}

CG_INLINE CGFloat
NMUI_EaseInBack(CGFloat t) {
    return pow(t, 3) - t * sin(t * M_PI);
}

CG_INLINE CGFloat
NMUI_EaseOutBack(CGFloat t) {
    CGFloat f = (1 - t);
    return 1 - (pow(f, 3) - f * sin(f * M_PI));
}

CG_INLINE CGFloat
NMUI_EaseInOutBack(CGFloat t) {
    if (t < 0.5) {
        CGFloat f = 2 * t;
        return 0.5 * (pow(f, 3) - f * sin(f * M_PI));
    } else {
        CGFloat f = (1 - (2 * t - 1));
        return 0.5 * (1 - (pow(f, 3) - f * sin(f * M_PI))) + 0.5;
    }
}

CG_INLINE CGFloat
NMUI_EaseInElastic(CGFloat t) {
    return sin(13 * M_PI_2 * t) * pow(2, 10 * (t - 1));
}

CG_INLINE CGFloat
NMUI_EaseOutElastic(CGFloat t) {
    return sin(-13 * M_PI_2 * (t + 1)) * pow(2, -10 * t) + 1;
}

CG_INLINE CGFloat
NMUI_EaseInOutElastic(CGFloat t) {
    if (t < 0.5) {
        return 0.5 * sin(13 * M_PI_2 * (2 * t)) * pow(2, 10 * ((2 * t) - 1));
    } else {
        return 0.5 * (sin(-13 * M_PI_2 * ((2 * t - 1) + 1)) * pow(2, -10 * (2 * t - 1)) + 2);
    }
}

CG_INLINE CGFloat
NMUI_EaseOutBounce(CGFloat t) {
    if (t < 4.0 / 11.0) {
        return (121.0 * t * t) / 16.0;
    } else if (t < 8.0 / 11.0) {
        return (363.0 / 40.0 * t * t) - (99.0 / 10.0 * t) + 17.0 / 5.0;
    } else if(t < 9.0 / 10.0) {
        return (4356.0 / 361.0 * t * t) - (35442.0 / 1805.0 * t) + 16061.0 / 1805.0;
    } else {
        return (54.0 / 5.0 * t * t) - (513.0 / 25.0 * t) + 268.0 / 25.0;
    }
}

CG_INLINE CGFloat
NMUI_EaseInBounce(CGFloat t) {
    return 1 - NMUI_EaseOutBounce(1 - t);
}

CG_INLINE CGFloat
NMUI_EaseInOutBounce(CGFloat t) {
    if (t < 0.5) {
        return 0.5 * NMUI_EaseInBounce(t * 2);
    } else {
        return 0.5 * NMUI_EaseOutBounce(t * 2 - 1) + 0.5;
    }
}

CG_INLINE CGFloat
NMUI_EaseSpring(CGFloat t, CGFloat mass, CGFloat damping, CGFloat stiffness, CGFloat initialVelocity) {
    
    // https://webkit.org/demos/spring/spring.js
    // https://webkit.org/demos/spring
    
    CGFloat m_w0 = sqrt(stiffness / mass);
    CGFloat m_zeta = damping / (2 * sqrt(stiffness * mass));
    
    CGFloat m_wd = 0;
    CGFloat m_A = 0;
    CGFloat m_B = 0;
    
    if (m_zeta < 1) {
        // Under-damped.
        m_wd = m_w0 * sqrt(1 - m_zeta * m_zeta);
        m_A = 1;
        m_B = (m_zeta * m_w0 + -initialVelocity) / m_wd;
    } else {
        // Critically damped (ignoring over-damped case for now).
        m_wd = 0;
        m_A = 1;
        m_B = -initialVelocity + m_w0;
    }
    
    if (m_zeta < 1) {
        // Under-damped
        t = exp(-t * m_zeta * m_w0) * (m_A * cos(m_wd * t) + m_B * sin(m_wd * t));
    } else {
        // Critically damped (ignoring over-damped case for now).
        t = (m_A + m_B * t) * exp(-t * m_w0);
    }
    
    // Map range from [1..0] to [0..1].
    return 1 - t;
}
