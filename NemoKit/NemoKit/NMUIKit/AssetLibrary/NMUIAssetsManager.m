...
void NMUIImageWriteToSavedPhotosAlbumWithAlbumAssetsGroup(UIImage *image, NMUIAssetsGroup *albumAssetsGroup, NMUIWriteAssetCompletionBlock completionBlock) {
    [[NMUIAssetsManager sharedInstance] saveImageWithImageRef:image.CGImage albumAssetsGroup:albumAssetsGroup orientation:image.imageOrientation completionBlock:completionBlock];
}

void NMUISaveImageAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *imagePath, NMUIAssetsGroup *albumAssetsGroup, NMUIWriteAssetCompletionBlock completionBlock) {
    [[NMUIAssetsManager sharedInstance] saveImageWithImagePathURL:[NSURL fileURLWithPath:imagePath] albumAssetsGroup:albumAssetsGroup completionBlock:completionBlock];
}

void NMUISaveVideoAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *videoPath, NMUIAssetsGroup *albumAssetsGroup, NMUIWriteAssetCompletionBlock completionBlock) {
    [[NMUIAssetsManager sharedInstance] saveVideoWithVideoPathURL:[NSURL fileURLWithPath:videoPath] albumAssetsGroup:albumAssetsGroup completionBlock:completionBlock];
}



@implementation NMUIAssetsManager {
    PHCachingImageManager *_phCachingImageManager;
}

+ (NMUIAssetsManager *)sharedInstance {
    static dispatch_once_t onceToken;
    static NMUIAssetsManager *instance = nil;
...
+ (NMUIAssetAuthorizationStatus)authorizationStatus {
    __block NMUIAssetAuthorizationStatus status;
    // 获取当前应用对照片的访问授权状态
    PHAuthorizationStatus authorizationStatus = [PHPhotoLibrary authorizationStatus];
    if (authorizationStatus == PHAuthorizationStatusRestricted || authorizationStatus == PHAuthorizationStatusDenied) {
        status = NMUIAssetAuthorizationStatusNotAuthorized;
    } else if (authorizationStatus == PHAuthorizationStatusNotDetermined) {
        status = NMUIAssetAuthorizationStatusNotDetermined;
    } else {
        status = NMUIAssetAuthorizationStatusAuthorized;
    }
    return status;
}

+ (void)requestAuthorization:(void(^)(NMUIAssetAuthorizationStatus status))handler {
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus phStatus) {
        NMUIAssetAuthorizationStatus status;
...
- (void)enumerateAllAlbumsWithAlbumContentType:(NMUIAlbumContentType)contentType showEmptyAlbum:(BOOL)showEmptyAlbum showSmartAlbumIfSupported:(BOOL)showSmartAlbumIfSupported usingBlock:(void (^)(NMUIAssetsGroup *resultAssetsGroup))enumerationBlock {
    ...
    // 创建一个 PHFetchOptions，用于 NMUIAssetsGroup 对资源的排序以及对内容类型进行控制
    PHFetchOptions *phFetchOptions = [PHPhotoLibrary createFetchOptionsWithAlbumContentType:contentType];
    
    // 遍历结果，生成对应的 NMUIAssetsGroup，并调用 enumerationBlock
    for (NSUInteger i = 0; i < [tempAlbumsArray count]; i++) {
        PHAssetCollection *phAssetCollection = [tempAlbumsArray objectAtIndex:i];
        NMUIAssetsGroup *assetsGroup = [[NMUIAssetsGroup alloc] initWithPHCollection:phAssetCollection fetchAssetsOptions:phFetchOptions];
        if (enumerationBlock) {
...
- (void)enumerateAllAlbumsWithAlbumContentType:(NMUIAlbumContentType)contentType usingBlock:(void (^)(NMUIAssetsGroup *resultAssetsGroup))enumerationBlock {
...
- (void)saveImageWithImageRef:(CGImageRef)imageRef albumAssetsGroup:(NMUIAssetsGroup *)albumAssetsGroup orientation:(UIImageOrientation)orientation completionBlock:(NMUIWriteAssetCompletionBlock)completionBlock {
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] addImageToAlbum:imageRef
        ...
        if (success) {
            ...
            NMUIAsset *asset = [[NMUIAsset alloc] initWithPHAsset:phAsset];
            completionBlock(asset, error);
        } else {
            NMBFLog(@"NMUIAssetLibrary", @"Get PHAsset of image error: %@", error);
            completionBlock(nil, error);
        }
    }];
}

- (void)saveImageWithImagePathURL:(NSURL *)imagePathURL albumAssetsGroup:(NMUIAssetsGroup *)albumAssetsGroup completionBlock:(NMUIWriteAssetCompletionBlock)completionBlock {
    PHAssetCollection *albumPhAssetCollection = albumAssetsGroup.phAssetCollection;
    // 把图片加入到指定的相册对应的 PHAssetCollection
...
- (void)saveVideoWithVideoPathURL:(NSURL *)videoPathURL albumAssetsGroup:(NMUIAssetsGroup *)albumAssetsGroup completionBlock:(NMUIWriteAssetCompletionBlock)completionBlock {
    PHAssetCollection *albumPhAssetCollection = albumAssetsGroup.phAssetCollection;
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] addVideoToAlbum:videoPathURL
        ...
        if (success) {
            ...
            NMBFLog(@"NMUIAssetLibrary", @"Get PHAsset of video Error: %@", error);
            completionBlock(nil, error);
        }
    }];
}
...
@implementation PHPhotoLibrary (NMUI)

+ (PHFetchOptions *)createFetchOptionsWithAlbumContentType:(NMUIAlbumContentType)contentType {
    PHFetchOptions *fetchOptions = [[PHFetchOptions alloc] init];
    // 根据输入的内容类型过滤相册内的资源
    switch (contentType) {
        case NMUIAlbumContentTypeOnlyPhoto:
            fetchOptions.predicate = [NSPredicate predicateWithFormat:@"mediaType = %i", PHAssetMediaTypeImage];
            break;
            
        case NMUIAlbumContentTypeOnlyVideo:
            fetchOptions.predicate = [NSPredicate predicateWithFormat:@"mediaType = %i",PHAssetMediaTypeVideo];
            break;
            
        case NMUIAlbumContentTypeOnlyAudio:
...
+ (NSArray *)fetchAllAlbumsWithAlbumContentType:(NMUIAlbumContentType)contentType showEmptyAlbum:(BOOL)showEmptyAlbum showSmartAlbum:(BOOL)showSmartAlbum {
    NSMutableArray *tempAlbumsArray = [[NSMutableArray alloc] init];
    
    // 创建一个 PHFetchOptions，用于创建 NMUIAssetsGroup 对资源的排序和类型进行控制
...
- (void)addImageToAlbum:(UIImage *)image imagePathURL:(NSURL *)imagePathURL albumAssetCollection:(PHAssetCollection *)albumAssetCollection completionHandler:(void(^)(BOOL success, NSDate *creationDate, NSError *error))completionHandler {
    ...
    [self performChanges:^{
        ...
        if (image) {
            ...
            NMBFLog(@"NMUIAssetLibrary", @"Creating asset with empty data");
            return;
        }
        ...
        if (!success) {
            NMBFLog(@"NMUIAssetLibrary", @"Creating asset of image error : %@", error);
        }
...
- (void)addVideoToAlbum:(NSURL *)videoPathURL albumAssetCollection:(PHAssetCollection *)albumAssetCollection completionHandler:(void(^)(BOOL success, NSDate *creationDate, NSError *error))completionHandler {
    ...
    [self performChanges:^{
        ...
        if (!success) {
            NMBFLog(@"NMUIAssetLibrary", @"Creating asset of video error: %@", error);
        }
