...
@interface NMUITableViewCell() <UIScrollViewDelegate>

@property(nonatomic, assign) BOOL initByTableView;
@property(nonatomic, assign, readwrite) NMUITableViewCellPosition cellPosition;
...
@property(nonatomic, strong) NMUIButton *defaultAccessoryButton;
@property(nonatomic, strong) UIView *defaultDetailDisclosureView;
@end

@implementation NMUITableViewCell
...
- (instancetype)initForTableView:(UITableView *)tableView withStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self = [self initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        // _isGroupedStyle 的值因为 parentTableView 的变化而变化，所以这里重新执行一次 didInitializeWithStyle: 里的 nmui_styledAsNMUITableViewCell
        [self didInitializeWithStyle:style];
    }
    return self;
}
...
- (void)setEnabled:(BOOL)enabled {
    if (_enabled != enabled) {
        if (enabled) {
            ...
            UIColor *titleLabelColor = self.nmui_styledTextLabelColor;
            if (titleLabelColor) {
...
- (void)initDefaultAccessoryButtonIfNeeded {
    if (!self.defaultAccessoryButton) {
        self.defaultAccessoryButton = [[NMUIButton alloc] init];
...
// 重写accessoryType，如果是UITableViewCellAccessoryDisclosureIndicator类型的，则使用 NMUIConfigurationTemplate.m 配置表里的图片
...
- (void)handleAccessoryButtonEvent:(NMUIButton *)detailButton {
    if ([self.nmui_tableView.delegate respondsToSelector:@selector(tableView:accessoryButtonTappedForRowWithIndexPath:)]) {
        [self.nmui_tableView.delegate tableView:self.nmui_tableView accessoryButtonTappedForRowWithIndexPath:[self.nmui_tableView nmui_indexPathForRowAtView:detailButton]];
    }
}

@end

@implementation NMUITableViewCell(NMUISubclassingHooks)

- (void)didInitializeWithStyle:(UITableViewCellStyle)style {
    self.initByTableView = NO;
    _cellPosition = NMUITableViewCellPositionNone;
    ...
    [self nmui_styledAsNMUITableViewCell];
}

- (void)updateCellAppearanceWithIndexPath:(NSIndexPath *)indexPath {
    // 子类继承
    if (indexPath && self.parentTableView) {
        NMUITableViewCellPosition position = [self.parentTableView nmui_positionForRowAtIndexPath:indexPath];
        self.cellPosition = position;
    } else {
        self.cellPosition = NMUITableViewCellPositionNone;
    }
}
