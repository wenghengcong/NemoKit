//
//  NMUICollectionViewPagingLayout.m
//  Nemo
//
//  Created by Hunt on 2019/11/9.
...
@interface NMUICollectionViewPagingLayout () {
...
@implementation NMUICollectionViewPagingLayout (DefaultStyle)
...
- (void)setDebug:(BOOL)debug {
    _debug = debug;
    if (self.style == NMUICollectionViewPagingLayoutStyleDefault && debug && !self.debugLayer) {
        self.debugLayer = [CALayer layer];
        [self.debugLayer nmui_removeDefaultAnimations];
...
@implementation NMUICollectionViewPagingLayout (ScaleStyle)
...
const CGFloat NMUICollectionViewPagingLayoutRotationRadiusAutomatic = -1.0;

@implementation NMUICollectionViewPagingLayout (RotationStyle)
...
@implementation NMUICollectionViewPagingLayout

- (instancetype)initWithStyle:(NMUICollectionViewPagingLayoutStyle)style {
    if (self = [super init]) {
        _style = style;
        ...
        self.rotationRadius = NMUICollectionViewPagingLayoutRotationRadiusAutomatic;
...
- (instancetype)init {
    return [self initWithStyle:NMUICollectionViewPagingLayoutStyleDefault];
}
...
- (void)prepareLayout {
    ...
    if (self.debugLayer) {
        if (self.scrollDirection == UICollectionViewScrollDirectionVertical) {
            self.debugLayer.frame = CGRectFlatMake(0, self.collectionView.nmui_contentInset.top + self.sectionInset.top + _finalItemSize.height / 2, CGRectGetWidth(self.collectionView.bounds), PixelOne);
        } else {
            self.debugLayer.frame = CGRectFlatMake(self.collectionView.nmui_contentInset.left + self.sectionInset.left + _finalItemSize.width / 2, 0, PixelOne, CGRectGetHeight(self.collectionView.bounds));
        }
    }
}

- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    if (self.style == NMUICollectionViewPagingLayoutStyleScale || self.style == NMUICollectionViewPagingLayoutStyleRotation) {
        return YES;
    }
...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (self.style == NMUICollectionViewPagingLayoutStyleDefault) {
    ...
    if (self.style == NMUICollectionViewPagingLayoutStyleScale) {
    ...
    if (self.style == NMUICollectionViewPagingLayoutStyleRotation) {
        if (self.rotationRadius == NMUICollectionViewPagingLayoutRotationRadiusAutomatic) {
...
- (CGPoint)targetContentOffsetForProposedContentOffset:(CGPoint)proposedContentOffset withScrollingVelocity:(CGPoint)velocity {
    ...
    UIEdgeInsets contentInset = self.collectionView.nmui_contentInset;
    ...
    if (self.scrollDirection == UICollectionViewScrollDirectionVertical) {
        ...
        if (proposedContentOffset.y < -contentInset.top || proposedContentOffset.y >= contentSize.height + contentInset.bottom - frameSize.height) {
            if (IOS_VERSION_NUMBER < 100000) {
                ...
                // https://github.com/Tencent/NMUI_iOS/issues/499
