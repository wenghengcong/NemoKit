//
//  NMUICollectionViewPagingLayout.h
//  Nemo
//
...
typedef NS_ENUM(NSInteger, NMUICollectionViewPagingLayoutStyle) {
    NMUICollectionViewPagingLayoutStyleDefault, // 普通模式，水平滑动
    NMUICollectionViewPagingLayoutStyleScale,   // 缩放模式，两边的item会小一点，逐渐向中间放大
    NMUICollectionViewPagingLayoutStyleRotation // 旋转模式，围绕底部某个点为中心旋转
};
 ...
 */
@interface NMUICollectionViewPagingLayout : UICollectionViewFlowLayout

- (instancetype)initWithStyle:(NMUICollectionViewPagingLayoutStyle)style NS_DESIGNATED_INITIALIZER;

@property(nonatomic, assign, readonly) NMUICollectionViewPagingLayoutStyle style;
...
@interface NMUICollectionViewPagingLayout (DefaultStyle)
...
@interface NMUICollectionViewPagingLayout (ScaleStyle)
...
extern const CGFloat NMUICollectionViewPagingLayoutRotationRadiusAutomatic;

@interface NMUICollectionViewPagingLayout (RotationStyle)

/**
 *  旋转卡片相关
 *  左右两个卡片最终旋转的角度有 rotationRadius * 90 计算出来
 *  rotationRadius表示旋转的半径
 *  @warning 仅当 style 为 NMUICollectionViewPagingLayoutStyleRotation 时才生效
 */
