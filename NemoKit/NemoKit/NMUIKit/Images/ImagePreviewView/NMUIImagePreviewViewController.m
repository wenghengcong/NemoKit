//
//  NMUIImagePreviewViewController.m
//  Nemo
//
...
const CGFloat NMUIImagePreviewViewControllerCornerRadiusAutomaticDimension = -1;

@implementation NMUIImagePreviewViewController (UIAppearance)
...
static NMUIImagePreviewViewController *imagePreviewViewControllerAppearance;
+ (nonnull instancetype)appearance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!imagePreviewViewControllerAppearance) {
            imagePreviewViewControllerAppearance = [[NMUIImagePreviewViewController alloc] init];
...
@implementation NMUIImagePreviewViewController
...
- (void)didInitialize {
    ...
    self.sourceImageCornerRadius = NMUIImagePreviewViewControllerCornerRadiusAutomaticDimension;
    
    _dismissingGestureEnabled = YES;
    
    if (imagePreviewViewControllerAppearance) {
        self.backgroundColor = [NMUIImagePreviewViewController appearance].backgroundColor;
    }
    
    // present style
    self.transitioningAnimator = [[NMUIImagePreviewViewTransitionAnimator alloc] init];
...
- (NMUIImagePreviewView *)imagePreviewView {
...
- (void)initSubviews {
    [super initSubviews];
    _imagePreviewView = [[NMUIImagePreviewView alloc] initWithFrame:self.view.bounds];
    [self.view addSubview:self.imagePreviewView];
}
BeginIgnoreDeprecatedWarning
- (void)viewDidLayoutSubviews {
    [super viewDidLayoutSubviews];
    self.imagePreviewView.nmui_frameApplyTransform = self.view.bounds;
    
    UIViewController *backendViewController = [self visibleViewControllerWithViewController:self.presentingViewController];
    self.canShowPresentingViewControllerWhenGesturing = [NMUIHelper interfaceOrientationMask:backendViewController.supportedInterfaceOrientations containsInterfaceOrientation:UIApplication.sharedApplication.statusBarOrientation];
}
EndIgnoreDeprecatedWarning

- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    if (self.nmui_isPresented) {
...
- (void)setPresentingStyle:(NMUIImagePreviewViewControllerTransitioningStyle)presentingStyle {
    _presentingStyle = presentingStyle;
    self.dismissingStyle = presentingStyle;
}

- (void)setTransitioningAnimator:(__kindof NMUIImagePreviewViewTransitionAnimator *)transitioningAnimator {
    _transitioningAnimator = transitioningAnimator;
    transitioningAnimator.imagePreviewViewController = self;
}

- (BOOL)prefersStatusBarHidden {
    if (self.nmui_visibleState < NMUIViewControllerDidAppear || self.nmui_visibleState >= NMUIViewControllerDidDisappear) {
        // 在 present/dismiss 动画过程中，都使用原界面的状态栏显隐状态
        if (self.presentingViewController) {
            self.originalStatusBarHidden = self.presentingViewController.nmui_prefersStatusBarHidden;
...
// 不使用 nmui_visibleViewControllerIfExist 是因为不想考虑 presentedViewController
