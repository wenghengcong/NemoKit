//
//  NMUIImagePreviewViewTransitionAnimator.m
//  Nemo
//
...
@implementation NMUIImagePreviewViewTransitionAnimator
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self.cornerRadiusMaskLayer nmui_removeDefaultAnimations];
        self.cornerRadiusMaskLayer.backgroundColor = [UIColor whiteColor].CGColor;
        
        self.animationEnteringBlock = ^(__kindof NMUIImagePreviewViewTransitionAnimator * _Nonnull animator, BOOL isPresenting, NMUIImagePreviewViewControllerTransitioningStyle style, CGRect sourceImageRect, NMUIZoomImageView * _Nonnull zoomImageView, id<UIViewControllerContextTransitioning>  _Nullable transitionContext) {
            
            UIView *previewView = animator.imagePreviewViewController.view;
            
            if (style == NMUIImagePreviewViewControllerTransitioningStyleFade) {
                
                previewView.alpha = isPresenting ? 0 : 1;
                
            } else if (style == NMUIImagePreviewViewControllerTransitioningStyleZoom) {
                ...
                CGFloat cornerRadius = animator.imagePreviewViewController.sourceImageCornerRadius == NMUIImagePreviewViewControllerCornerRadiusAutomaticDimension && animator.imagePreviewViewController.sourceImageView ? animator.imagePreviewViewController.sourceImageView().layer.cornerRadius : MAX(animator.imagePreviewViewController.sourceImageCornerRadius, 0);
        ...
        self.animationBlock = ^(__kindof NMUIImagePreviewViewTransitionAnimator * _Nonnull animator, BOOL isPresenting, NMUIImagePreviewViewControllerTransitioningStyle style, CGRect sourceImageRect, NMUIZoomImageView * _Nonnull zoomImageView, id<UIViewControllerContextTransitioning>  _Nullable transitionContext) {
            if (style == NMUIImagePreviewViewControllerTransitioningStyleFade) {
        ...
        self.animationCompletionBlock = ^(__kindof NMUIImagePreviewViewTransitionAnimator * _Nonnull animator, BOOL isPresenting, NMUIImagePreviewViewControllerTransitioningStyle style, CGRect sourceImageRect, NMUIZoomImageView * _Nonnull zoomImageView, id<UIViewControllerContextTransitioning>  _Nullable transitionContext) {
...
- (void)animateTransition:(nonnull id<UIViewControllerContextTransitioning>)transitionContext {
    ...
    NMUIImagePreviewViewControllerTransitioningStyle style = isPresenting ? self.imagePreviewViewController.presentingStyle : self.imagePreviewViewController.dismissingStyle;
    CGRect sourceImageRect = CGRectZero;
    if (style == NMUIImagePreviewViewControllerTransitioningStyleZoom) {
    ...
    style = style == NMUIImagePreviewViewControllerTransitioningStyleZoom && CGRectEqualToRect(sourceImageRect, CGRectZero) ? NMUIImagePreviewViewControllerTransitioningStyleFade : style;// zoom 类型一定需要有个非 zero 的 sourceImageRect，否则不知道动画的起点/终点，所以当不存在 sourceImageRect 时强制改为用 fade 动画
    
    UIView *containerView = transitionContext.containerView;
    UIView *fromView = [transitionContext viewForKey:UITransitionContextFromViewKey];
    UIView *toView = [transitionContext viewForKey:UITransitionContextToViewKey];
    NMUIZoomImageView *zoomImageView = [self.imagePreviewViewController.imagePreviewView zoomImageViewAtIndex:self.imagePreviewViewController.imagePreviewView.currentImageIndex];
