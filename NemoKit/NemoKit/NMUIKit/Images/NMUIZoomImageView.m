...
// generate icon images needed by NMUIZoomImageView
// 用于生成 NMUIZoomImageView 所需的一些简单的图标图片
@interface NMUIZoomImageViewImageGenerator : NSObject
...
@interface NMUIZoomImageVideoPlayerView : UIView

@end

static NSUInteger const kTagForCenteredPlayButton = 1;

@interface NMUIZoomImageView () <UIGestureRecognizerDelegate>

// video play
@property(nonatomic, strong) NMUIZoomImageVideoPlayerView *videoPlayerView;
...
@implementation NMUIZoomImageView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _scrollView = [[UIScrollView alloc] nmui_initWithSize:frame.size];
        ...
        _emptyView = [[NMUIEmptyView alloc] init];
...
- (void)layoutSubviews {
    ...
    if (_videoToolbar) {
        _videoToolbar.frame = ({
            UIEdgeInsets margins = UIEdgeInsetsConcat(self.videoToolbarMargins, self.nmui_safeAreaInsets);
...
- (void)setImage:(UIImage *)image {
    ...
    self.imageView.nmui_frameApplyTransform = CGRectMakeWithSize(image.size);
...
- (void)setLivePhoto:(PHLivePhoto *)livePhoto {
    ...
    _livePhotoView.nmui_frameApplyTransform = CGRectMakeWithSize(livePhoto.size);
...
- (void)revertZooming {
    ...
    self.scrollView.maximumZoomScale = maximumZoomScale;
    self.contentView.frame = CGRectSetXY(self.contentView.frame, 0, 0);// 重置 origin 的目的是：https://github.com/Tencent/NMUI_iOS/issues/400
...
- (void)setZoomScale:(CGFloat)zoomScale animated:(BOOL)animated {
    if (animated) {
        [UIView animateWithDuration:.25 delay:0.0 options:NMUIViewAnimationOptionsCurveOut animations:^{
...
- (void)setVideoPlayerItem:(AVPlayerItem *)videoPlayerItem {
    ...
    self.videoPlayerView.nmui_frameApplyTransform = CGRectMakeWithSize(self.videoSize);
...
- (void)addPlayerTimeObserver {
    ...
    double interval = .1f;
    __weak NMUIZoomImageView *weakSelf = self;
...
- (NMUIZoomImageViewVideoToolbar *)videoToolbar {
    [self initVideoToolbarIfNeeded];
    return _videoToolbar;
}

- (NMUIButton *)videoCenteredPlayButton {
...
- (void)initVideoPlayerLayerIfNeeded {
    ...
    self.videoPlayerView = [[NMUIZoomImageVideoPlayerView alloc] init];
...
- (void)initVideoToolbarIfNeeded {
    ...
    _videoToolbar = ({
        NMUIZoomImageViewVideoToolbar * b = [[NMUIZoomImageViewVideoToolbar alloc] init];
...
- (void)initVideoCenteredPlayButtonIfNeeded {
    ...
    _videoCenteredPlayButton = ({
        NMUIButton *b = [[NMUIButton alloc] init];
        b.nmui_outsideEdge = UIEdgeInsetsMake(-60, -60, -60, -60);
...
- (NMUIPieProgressView *)cloudProgressView {
...
- (void)initCloudProgressViewIfNeeded {
    ...
    _cloudProgressView = [[NMUIPieProgressView alloc] init];
...
- (void)initCloudDownloadRetryButtonIfNeeded {
    ...
    _cloudDownloadRetryButton = [[NMUIButton alloc] init];
    [_cloudDownloadRetryButton setImage:[NMUIHelper imageWithName:@"NMUI_icloud_download_fault"] forState:UIControlStateNormal];
    ...
    _cloudDownloadRetryButton.nmui_outsideEdge = UIEdgeInsetsMake(-6, -6, -6, -6);
...
- (void)setCloudDownloadStatus:(NMUIAssetDownloadStatus)cloudDownloadStatus {
...
@interface NMUIZoomImageView (UIAppearance)

@end

@implementation NMUIZoomImageView (UIAppearance)
...
+ (void)setDefaultAppearance {
    NMUIZoomImageView *appearance = [NMUIZoomImageView appearance];
    appearance.videoToolbarMargins = UIEdgeInsetsMake(0, 25, 25, 18);
    appearance.videoCenteredPlayButtonImage = [NMUIZoomImageViewImageGenerator largePlayImage];
}

@end

@implementation NMUIZoomImageVideoPlayerView
...
@implementation NMUIZoomImageViewImageGenerator

+ (UIImage *)largePlayImage {
    CGFloat width = 60;
    return [UIImage nmui_imageWithSize:CGSizeMake(width, width) opaque:NO scale:0 actions:^(CGContextRef contextRef) {
...
@implementation NMUIZoomImageViewVideoToolbar

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        
        _playButton = [[NMUIButton alloc] init];
        self.playButton.nmui_outsideEdge = UIEdgeInsetsMake(-10, -10, -10, -10);
        [self.playButton setImage:self.playButtonImage forState:UIControlStateNormal];
        [self addSubview:self.playButton];
        
        _pauseButton = [[NMUIButton alloc] init];
        self.pauseButton.nmui_outsideEdge = UIEdgeInsetsMake(-10, -10, -10, -10);
        [self.pauseButton setImage:self.pauseButtonImage forState:UIControlStateNormal];
        [self addSubview:self.pauseButton];
        
        _slider = [[NMUISlider alloc] init];
        ...
        _sliderLeftLabel = [[UILabel alloc] nmui_initWithFont:UIFontMake(12) textColor:UIColorWhite];
        ...
        [self.sliderRightLabel nmui_setTheSameAppearanceAsLabel:self.sliderLeftLabel];
...
@interface NMUIZoomImageViewVideoToolbar (UIAppearance)

@end

@implementation NMUIZoomImageViewVideoToolbar (UIAppearance)
...
+ (void)setDefaultAppearance {
    NMUIZoomImageViewVideoToolbar *appearance = [NMUIZoomImageViewVideoToolbar appearance];
    appearance.playButtonImage = [NMUIZoomImageViewImageGenerator smallPlayImage];
    appearance.pauseButtonImage = [NMUIZoomImageViewImageGenerator pauseImage];
}
