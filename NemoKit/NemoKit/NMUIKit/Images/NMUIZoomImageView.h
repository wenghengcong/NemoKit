...
@class NMUIZoomImageView;
@class NMUIEmptyView;
@class NMUIButton;
@class NMUISlider;
@class NMUIZoomImageViewVideoToolbar;
@class NMUIPieProgressView;

@protocol NMUIZoomImageViewDelegate <NSObject>
@optional
- (void)singleTouchInZoomingImageView:(NMUIZoomImageView *)zoomImageView location:(CGPoint)location;
- (void)doubleTouchInZoomingImageView:(NMUIZoomImageView *)zoomImageView location:(CGPoint)location;
- (void)longPressInZoomingImageView:(NMUIZoomImageView *)zoomImageView;
...
/// 是否支持缩放，默认为 YES
- (BOOL)enabledZoomViewInZoomImageView:(NMUIZoomImageView *)zoomImageView;
 ...
 *  @see viewportRect
 *
 *  NMUIZoomImageView 提供最基础的图片预览和缩放功能以及 loading、错误等状态的展示支持，其他功能请通过继承来实现。
 */
@interface NMUIZoomImageView : UIView <UIScrollViewDelegate>

@property(nonatomic, weak) id<NMUIZoomImageViewDelegate> delegate;
...
// @see NMUIZoomImageViewVideoToolbar
@property(nonatomic, strong, readonly) NMUIZoomImageViewVideoToolbar *videoToolbar;

// 视频底部控制条的 margins，会在此基础上自动叠加 NMUIZoomImageView.nmui_safeAreaInsets，因此无需考虑在 iPhone X 下的兼容
// 默认值为 {0, 25, 25, 18}
@property(nonatomic, assign) UIEdgeInsets videoToolbarMargins UI_APPEARANCE_SELECTOR;

// 播放 video 时屏幕中央的播放按钮
@property(nonatomic, strong, readonly) NMUIButton *videoCenteredPlayButton;

// 可通过此属性修改 video 播放时屏幕中央的播放按钮图片
@property(nonatomic, strong) UIImage *videoCenteredPlayButtonImage UI_APPEARANCE_SELECTOR;

// 从 iCloud 加载资源的进度展示
@property(nonatomic, strong) NMUIPieProgressView *cloudProgressView;

// 从 iCloud 加载资源失败的重试按钮
@property(nonatomic, strong) NMUIButton *cloudDownloadRetryButton;

// 当前展示的资源的下载状态
@property(nonatomic, assign) NMUIAssetDownloadStatus cloudDownloadStatus;
...
/**
 *  获取当前正在显示的图片/视频在整个 NMUIZoomImageView 坐标系里的 rect（会按照当前的缩放状态来计算）
 */
...
@property(nonatomic, strong, readonly) NMUIEmptyView *emptyView;
...
@interface NMUIZoomImageViewVideoToolbar : UIView

@property(nonatomic, strong, readonly) NMUIButton *playButton;
@property(nonatomic, strong, readonly) NMUIButton *pauseButton;
@property(nonatomic, strong, readonly) NMUISlider *slider;
