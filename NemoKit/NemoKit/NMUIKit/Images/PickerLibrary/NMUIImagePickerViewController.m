//
//  NMUIImagePickerViewController.m
//  Nemo
//
...
#pragma mark - NMUIImagePickerViewController (UIAppearance)

@implementation NMUIImagePickerViewController (UIAppearance)
...
static NMUIImagePickerViewController *imagePickerViewControllerAppearance;
+ (nonnull instancetype)appearance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!imagePickerViewControllerAppearance) {
            imagePickerViewControllerAppearance = [[NMUIImagePickerViewController alloc] init];
...
#pragma mark - NMUIImagePickerViewController

@interface NMUIImagePickerViewController ()

@property(nonatomic, strong) NMUIImagePickerPreviewViewController *imagePickerPreviewViewController;
...
@implementation NMUIImagePickerViewController

- (void)didInitialize {
    [super didInitialize];
    if (imagePickerViewControllerAppearance) {
        // 避免 imagePickerViewControllerAppearance init 时走到这里来，导致死循环
        self.minimumImageWidth = [NMUIImagePickerViewController appearance].minimumImageWidth;
    }
...
- (void)initSubviews {
    ...
    [self.collectionView registerClass:[NMUIImagePickerCollectionViewCell class] forCellWithReuseIdentifier:kVideoCellIdentifier];
    [self.collectionView registerClass:[NMUIImagePickerCollectionViewCell class] forCellWithReuseIdentifier:kImageOrUnknownCellIdentifier];
    if (@available(iOS 11, *)) {
    ...
    if (self.allowsMultipleSelection) {
        ...
        self.operationToolBarView.nmui_borderPosition = NMUIViewBorderPositionTop;
        [self.view addSubview:self.operationToolBarView];
        
        _sendButton = [[NMUIButton alloc] init];
        ...
        self.sendButton.nmui_outsideEdge = UIEdgeInsetsMake(-12, -20, -12, -20);
        ...
        _previewButton = [[NMUIButton alloc] init];
        ...
        self.previewButton.nmui_outsideEdge = UIEdgeInsetsMake(-12, -20, -12, -20);
...
- (void)setupNavigationItems {
    ...
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem nmui_itemWithTitle:@"取消" target:self action:@selector(handleCancelPickerImage:)];
}
...
- (void)viewDidLayoutSubviews {
    ...
    UIEdgeInsets contentInset = UIEdgeInsetsMake(self.nmui_navigationBarMaxYInViewCoordinator, self.collectionView.nmui_safeAreaInsets.left, MAX(operationToolBarViewHeight, self.collectionView.nmui_safeAreaInsets.bottom), self.collectionView.nmui_safeAreaInsets.right);
...
- (void)refreshWithAssetsGroup:(NMUIAssetsGroup *)assetsGroup {
    ...
    // 通过 NMUIAssetsGroup 获取该相册所有的图片 NMUIAsset，并且储存到数组中
    NMUIAlbumSortType albumSortType = NMUIAlbumSortTypePositive;
    // 从 delegate 中获取相册内容的排序方式，如果没有实现这个 delegate，则使用 NMUIAlbumSortType 的默认值，即最新的内容排在最后面
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [assetsGroup enumerateAssetsWithOptions:albumSortType usingBlock:^(NMUIAsset *resultAsset) {
...
- (void)scrollToInitialPositionIfNeeded {
    if (self.collectionView.nmui_visible && self.isImagesAssetLoaded && !self.hasScrollToInitialPosition) {
        if ([self.imagePickerViewControllerDelegate respondsToSelector:@selector(albumSortTypeForImagePickerViewController:)] && [self.imagePickerViewControllerDelegate albumSortTypeForImagePickerViewController:self] == NMUIAlbumSortTypeReverse) {
            [self.collectionView nmui_scrollToTop];
        } else {
            [self.collectionView nmui_scrollToBottom];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    NMUIAsset *imageAsset = [self.imagesAssetArray objectAtIndex:indexPath.item];
    ...
    NMUIImagePickerCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
    ...
    if (cell.selectable) {
        // 如果该图片的 NMUIAsset 被包含在已选择图片的数组中，则控制该图片被选中
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    NMUIAsset *imageAsset = self.imagesAssetArray[indexPath.item];
...
- (void)handleCheckBoxButtonClick:(UIButton *)checkboxButton {
    NSIndexPath *indexPath = [self.collectionView nmui_indexPathForItemAtView:checkboxButton];
    ...
    NMUIImagePickerCollectionViewCell *cell = (NMUIImagePickerCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
    NMUIAsset *imageAsset = [self.imagesAssetArray objectAtIndex:indexPath.item];
    if (cell.checked) {
        // 移除选中状态
        ...
        if ([self.selectedImageAssetArray count] >= _maximumSelectImageCount) {
            ...
            NMUIAlertController *alertController = [NMUIAlertController alertControllerWithTitle:_alertTitleWhenExceedMaxSelectImageCount message:nil preferredStyle:NMUIAlertControllerStyleAlert];
            [alertController addAction:[NMUIAlertAction actionWithTitle:_alertButtonTitleWhenExceedMaxSelectImageCount style:NMUIAlertActionStyleCancel handler:nil]];
...
- (void)updateImageCountAndCheckLimited {
    ...
    if (selectedImageCount > 0 && selectedImageCount >= _minimumSelectImageCount) {
        ...
        self.imageCountLabel.text = [NSString stringWithFormat:@"%@", @(selectedImageCount)];
        self.imageCountLabel.hidden = NO;
        [NMUIImagePickerHelper springAnimationOfImageSelectedCountChangeWithCountLabel:self.imageCountLabel];
    } else {
        self.previewButton.enabled = NO;
        self.sendButton.enabled = NO;
        self.imageCountLabel.hidden = YES;
...
- (void)requestImageWithIndexPath:(NSIndexPath *)indexPath {
    ...
    imageAsset.requestID = [imageAsset requestOriginImageWithCompletion:^(UIImage *result, NSDictionary *info) {
        ...
        if (downloadSucceed) {
            ...
            cell.downloadStatus = NMUIAssetDownloadStatusSucceed;
            
        } else if ([info objectForKey:PHImageErrorKey] ) {
            // 下载错误
            [imageAsset updateDownloadStatusWithDownloadResult:NO];
            cell.downloadStatus = NMUIAssetDownloadStatusFailed;
        }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.collectionView nmui_itemVisibleAtIndexPath:indexPath]) {
                
                NMBFLogInfo(@"NMUIImagePickerLibrary", @"Download iCloud image, current progress is : %f", progress);
                
                if (cell.downloadStatus != NMUIAssetDownloadStatusDownloading) {
                    cell.downloadStatus = NMUIAssetDownloadStatusDownloading;
                    // 预先设置预览界面的下载状态
                    self.imagePickerPreviewViewController.downloadStatus = NMUIAssetDownloadStatusDownloading;
                }
                if (error) {
