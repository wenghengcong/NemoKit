...
static NSString * const kLastAlbumKeyPrefix = @"NMUILastestAlbumKeyWith";
static NSString * const kContentTypeOfLastAlbumKeyPrefix = @"NMUIContentTypeOfLastestAlbumKeyWith";

@implementation NMUIImagePickerHelper
...
+ (NMUIAssetsGroup *)assetsGroupOfLastPickerAlbumWithUserIdentify:(NSString *)userIdentify {
    ...
    // 使用特定的前缀和可以标记不同用户的字符串拼接成 key，用于获取当前用户最近调用 updateLastestAlbumWithAssetsGroup 储存的相册以及对于的 NMUIAlbumContentType 值
    ...
    __block NMUIAssetsGroup *assetsGroup;
    
    NMUIAlbumContentType albumContentType = (NMUIAlbumContentType)[userDefaults integerForKey:contentTypeOflastAlbumKey];
     ...
     *  在 NMUI 2.0.0 及较早的版本中，NMUI 兼容 AssetsLibrary 的使用，
    ...
    if (groupIdentifier && [groupIdentifier isKindOfClass:[NSString class]]) {
        ...
        if (phFetchResult.count > 0) {
            ...
            assetsGroup = [[NMUIAssetsGroup alloc] initWithPHCollection:phAssetCollection fetchAssetsOptions:phFetchOptions];
        }
    } else {
        NMBFLog(@"NMUIImagePickerLibrary", @"Group For localIdentifier is not found! groupIdentifier is %@", groupIdentifier);
    }
    return assetsGroup;
}
...
+ (void)updateLastestAlbumWithAssetsGroup:(NMUIAssetsGroup *)assetsGroup ablumContentType:(NMUIAlbumContentType)albumContentType userIdentify:(NSString *)userIdentify {
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    // 使用特定的前缀和可以标记不同用户的字符串拼接成 key，用于为当前用户储存相册对应的 NMUIAssetsGroup 与 NMUIAlbumContentType
    NSString *lastAlbumKey = [NSString stringWithFormat:@"%@%@", kLastAlbumKeyPrefix, userIdentify];
    NSString *contentTypeOflastAlbumKey = [NSString stringWithFormat:@"%@%@", kContentTypeOfLastAlbumKeyPrefix, userIdentify];
...
+ (BOOL)imageAssetsDownloaded:(NSMutableArray<NMUIAsset *> *)imagesAssetArray {
    for (NMUIAsset *asset in imagesAssetArray) {
        if (asset.downloadStatus != NMUIAssetDownloadStatusSucceed) {
            return NO;
        }
    }
    return YES;
...
+ (void)requestImageAssetIfNeeded:(NMUIAsset *)asset completion: (void (^)(NMUIAssetDownloadStatus downloadStatus, NSError *error))completion {
    if (asset.downloadStatus != NMUIAssetDownloadStatusSucceed) {
        ...
        // 资源加载中
        if (completion) {
            completion(NMUIAssetDownloadStatusDownloading, nil);
        }
        ...
        [asset requestOriginImageWithCompletion:^(UIImage *result, NSDictionary<NSString *,id> *info) {
            ...
            if (downloadSucceed) {
                ...
                if (completion) {
                    completion(NMUIAssetDownloadStatusSucceed, nil);
                }
                ...
                if (completion) {
                    completion(NMUIAssetDownloadStatusFailed, [info objectForKey:PHImageErrorKey]);
                }
            }
        } withProgressHandler:^(double progress, NSError * _Nullable error, BOOL * _Nonnull stop, NSDictionary * _Nullable info) {
            NMBFLog(@"NMUIImagePickerLibrary", @"current progress is %f", progress);
