//
//  NMUIModalPresentationViewController.m
//  Nemo
//
...
@property(nonatomic, weak, readwrite) NMUIModalPresentationViewController *nmui_modalPresentationViewController;
@end

@implementation NMUIModalPresentationViewController (UIAppearance)

static NMUIModalPresentationViewController *appearance;
...
+ (void)initDefaultAppearance {
    ...
    appearance.animationStyle = NMUIModalPresentationAnimationStyleFade;
...
@interface NMUIModalPresentationViewController ()<NMUIKeyboardManagerDelegate>

@property(nonatomic, strong) NMUIModalPresentationWindow *containerWindow;
...
@property(nonatomic, strong) NMUIKeyboardManager *keyboardManager;
@property(nonatomic, assign) CGFloat keyboardHeight;
@end

@implementation NMUIModalPresentationViewController
...
- (void)didInitialize {
    if (appearance) {
        ...
        if (self != appearance) {
            self.keyboardManager = [[NMUIKeyboardManager alloc] initWithDelegate:self];
...
- (void)viewDidLayoutSubviews {
    ...
    if (self.layoutBlock) {
        ...
        self.contentView.nmui_frameApplyTransform = contentViewFrame;
    }
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.contentViewController) {
        self.contentViewController.nmui_modalPresentationViewController = self;
    ...
    if (self.isShownInWindowMode) {
        [NMUIHelper dimmedApplicationWindow];
    }
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (self.isShownInWindowMode) {
        [NMUIHelper resetDimmedApplicationWindow];
    }
    ...
    void (^didHiddenCompletion)(BOOL finished) = ^(BOOL finished) {
        ...
        if (self.contentViewController) {
            self.contentViewController.nmui_modalPresentationViewController = nil;
...
- (void)setContentViewController:(UIViewController<NMUIModalPresentationContentViewControllerProtocol> *)contentViewController {
...
- (void)showingAnimationWithCompletion:(void (^)(BOOL))completion {
    if (self.animationStyle == NMUIModalPresentationAnimationStyleFade) {
        self.dimmingView.alpha = 0.0;
        self.contentView.alpha = 0.0;
        [UIView animateWithDuration:.2 delay:0.0 options:NMUIViewAnimationOptionsCurveOut animations:^{
    ...
    } else if (self.animationStyle == NMUIModalPresentationAnimationStylePopup) {
        self.dimmingView.alpha = 0.0;
        self.contentView.transform = CGAffineTransformMakeScale(0, 0);
        [UIView animateWithDuration:.3 delay:0.0 options:NMUIViewAnimationOptionsCurveOut animations:^{
            self.dimmingView.alpha = 1.0;
    ...
    } else if (self.animationStyle == NMUIModalPresentationAnimationStyleSlide) {
        self.dimmingView.alpha = 0.0;
...
- (void)showWithAnimated:(BOOL)animated completion:(void (^)(BOOL))completion {
    ...
    if (!self.containerWindow) {
        self.containerWindow = [[NMUIModalPresentationWindow alloc] init];
        self.containerWindow.nmui_capturesStatusBarAppearance = NO;// modalPrensetationViewController.contentViewController 默认无权管理状态栏的样式，如需修改状态栏，请业务自己将这个属性改为 YES
        self.containerWindow.windowLevel = UIWindowLevelNMUIAlertView;
...
@implementation NMUIModalPresentationViewController (Manager)

+ (BOOL)isAnyModalPresentationViewControllerVisible {
    for (UIWindow *window in UIApplication.sharedApplication.windows) {
        if ([window isKindOfClass:[NMUIModalPresentationWindow class]] && !window.hidden) {
            return YES;
        }
    }
    return NO;
...
+ (BOOL)hideAllVisibleModalPresentationViewControllerIfCan {
    ...
    for (UIWindow *window in UIApplication.sharedApplication.windows) {
        if (![window isKindOfClass:[NMUIModalPresentationWindow class]]) {
            continue;
        }
        ...
        NMUIModalPresentationViewController *modalViewController = (NMUIModalPresentationViewController *)window.rootViewController;
        BOOL canHide = YES;
        ...
        if (canHide) {
            // 如果某些控件的显隐能力是通过 NMUIModalPresentationViewController 实现的，那么隐藏它们时，应该用它们自己的 hide 方法，而不是 NMUIModalPresentationViewController 自带的 hideWithAnimated:completion:
            id<NMUIModalPresentationComponentProtocol> modalPresentationComponent = nil;
            if ([modalViewController.contentViewController conformsToProtocol:@protocol(NMUIModalPresentationComponentProtocol)]) {
                modalPresentationComponent = (id<NMUIModalPresentationComponentProtocol>)modalViewController.contentViewController;
            } else if ([modalViewController.contentView conformsToProtocol:@protocol(NMUIModalPresentationComponentProtocol)]) {
                modalPresentationComponent = (id<NMUIModalPresentationComponentProtocol>)modalViewController.contentView;
            }
...
@implementation NMUIModalPresentationWindow
BeginIgnoreDeprecatedWarning
- (void)layoutSubviews {
...
@implementation UIViewController (NMUIModalPresentationViewController)

NMBFSynthesizeIdWeakProperty(nmui_modalPresentationViewController, setNmui_modalPresentationViewController)
