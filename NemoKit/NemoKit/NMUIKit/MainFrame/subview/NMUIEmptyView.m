...
- (void)didInitialize {
    ...
    NMUIEmptyView *appearance = [NMUIEmptyView appearance];
    ...
    _loadingView = (UIView<NMUIEmptyViewLoadingViewProtocol> *)[[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle: UIActivityIndicatorViewStyleGray];
    ...
    _actionButton = [[NMUIButton alloc] init];
    self.actionButton.nmui_outsideEdge = UIEdgeInsetsMake(-20, -20, -20, -20);
    self.actionButton.nmui_automaticallyAdjustTouchHighlightedInScrollView = YES;
...
- (void)layoutSubviews {
    ...
    if (!self.textLabel.hidden) {
        self.textLabel.frame = CGRectFlatMake(self.textLabelInsets.left, originY + self.textLabelInsets.top, CGRectGetWidth(self.contentView.bounds) - UIEdgeInsetsGetHorizontalValue(self.textLabelInsets), NMUIViewSelfSizingHeight);
        originY = CGRectGetMaxY(self.textLabel.frame) + self.textLabelInsets.bottom;
    }
    
    if (!self.detailTextLabel.hidden) {
        self.detailTextLabel.frame = CGRectFlatMake(self.detailTextLabelInsets.left, originY + self.detailTextLabelInsets.top, CGRectGetWidth(self.contentView.bounds) - UIEdgeInsetsGetHorizontalValue(self.detailTextLabelInsets), NMUIViewSelfSizingHeight);
...
- (void)updateDetailTextLabelWithText:(NSString *)text {
    if (self.detailTextLabelFont && self.detailTextLabelTextColor && text) {
        NSAttributedString *string = [[NSAttributedString alloc] initWithString:text attributes:@{
            ...
            NSParagraphStyleAttributeName: [NSMutableParagraphStyle nmui_paragraphStyleWithLineHeight:self.detailTextLabelFont.pointSize + 10 lineBreakMode:NSLineBreakByWordWrapping textAlignment:NSTextAlignmentCenter]
        }];
...
- (void)setLoadingView:(UIView<NMUIEmptyViewLoadingViewProtocol> *)loadingView {
...
@interface NMUIEmptyView (UIAppearance)

@end

@implementation NMUIEmptyView (UIAppearance)
