...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        NMBFOverrideImplementation([UINavigationBar class], @selector(layoutSubviews), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UINavigationBar *selfObject) {
                
                NMUINavigationTitleView *titleView = (NMUINavigationTitleView *)selfObject.topItem.titleView;
                
                if ([titleView isKindOfClass:[NMUINavigationTitleView class]]) {
...
@interface NMUINavigationTitleView ()
...
@implementation NMUINavigationTitleView

#pragma mark - 初始化

- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithStyle:NMUINavigationTitleViewStyleDefault frame:frame];
}
...
- (instancetype)initWithStyle:(NMUINavigationTitleViewStyle)style frame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.accessoryType = NMUINavigationTitleViewAccessoryTypeNone;
        
        NMUINavigationTitleView *appearance = [NMUINavigationTitleView appearance];
        ...
        self.tintColor = NMUICMIActivated ? NavBarTitleColor : [UINavigationBar appearance].titleTextAttributes[NSForegroundColorAttributeName];
    }
    return self;
}
...
- (CGSize)contentSize {
    ...
    if (self.style == NMUINavigationTitleViewStyleSubTitleVertical) {
...
- (void)setHorizontalTitleFont:(UIFont *)horizontalTitleFont {
    ...
    if (self.style == NMUINavigationTitleViewStyleDefault) {
...
- (void)setAccessoryType:(NMUINavigationTitleViewAccessoryType)accessoryType {
    
    // 如果已设置了accessoryView，则accessoryType不生效
    if (self.accessoryView) {
        accessoryType = NMUINavigationTitleViewAccessoryTypeNone;
    }
    
    _accessoryType = accessoryType;
    
    if (accessoryType == NMUINavigationTitleViewAccessoryTypeNone) {
    ...
    if (accessoryType == NMUINavigationTitleViewAccessoryTypeDisclosureIndicator) {
        accessoryImage = [NavBarAccessoryViewTypeDisclosureIndicatorImage nmui_imageWithOrientation:UIImageOrientationUp];
    }
...
- (void)updateSubAccessoryViewHidden {
    if (self.subAccessoryView && self.subtitle.length && self.style == NMUINavigationTitleViewStyleSubTitleVertical) {
...
- (void)setActive:(BOOL)active {
    ...
    if (self.accessoryType == NMUINavigationTitleViewAccessoryTypeDisclosureIndicator) {
        if (active) {
            [UIView animateWithDuration:.25f delay:0 options:NMUIViewAnimationOptionsCurveIn animations:^(void){
...
- (void)setStyle:(NMUINavigationTitleViewStyle)style {
    _style = style;
    if (style == NMUINavigationTitleViewStyleSubTitleVertical) {
...
@interface NMUINavigationTitleView (UIAppearance)

@end

@implementation NMUINavigationTitleView (UIAppearance)
...
@implementation NMUINavigationTitleView (LargeTitleCompatibility)

- (void)setAlpha:(BOOL)alpha animated:(BOOL)animated {
    // 在 push 和 pop 过渡期间系统会对自定义的 titleView 的 alpha 进行调整，了避免和系统的设置冲突（比如设置 alpha 为 0 又被系统还原为 1）这里通过设置 contentView 的 alpha 来控制整个 NMUINavigationTitleView 显示与隐藏。
    [UIView nmui_animateWithAnimated:animated duration:0.25f animations:^{
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        if (@available(iOS 11.0, *)) {
            NMBFOverrideImplementation([UINavigationController class], sel_registerName("_updateTopViewFramesToMatchScrollOffsetInViewController:contentScrollView:topLayoutType:"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UINavigationController *selfObject, UIViewController *viewController, UIScrollView *scrollView, NSUInteger topLayoutType) {
                    ...
                    [selfObject nmui_updateTitleViewToMatchScrollOffsetInViewController:viewController contentScrollView:scrollView topLayoutType:topLayoutType];
                };
            });
        }
    });
...
- (void)nmui_updateTitleViewToMatchScrollOffsetInViewController:(UIViewController *)viewController contentScrollView:(UIScrollView *)contentScrollView topLayoutType:(NSInteger)topLayoutType {
    if (@available(iOS 11.0, *)) {
        UIView *titleView = viewController.navigationItem.titleView;
        if (!titleView || ![titleView isKindOfClass:[NMUINavigationTitleView class]]) {
            return;
        }
        
        NSAssert(viewController.navigationController == self, @"navigationController is nil");
        
        NMUINavigationTitleView *navigationTitleView = (NMUINavigationTitleView *)titleView;
        UIView *largeTitleView = self.navigationBar.nmui_largeTitleView;
        BOOL largeTitleLabelVisable = self.navigationBar.prefersLargeTitles && viewController.nmui_prefersLargeTitleDisplayed && largeTitleView.alpha != 0;
