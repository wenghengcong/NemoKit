 ...
 *
 *  1. 自带顶部标题控件 `NMUINavigationTitleView`，支持loading、副标题、下拉菜单，设置标题依然使用系统的 `setTitle:` 方法
 *
 *  2. 自带空界面控件 `NMUIEmptyView`，支持显示loading、空文案、操作按钮
 *
 ...
 *
 *  5. 配合 `NMUINavigationController` 使用时，可以得到 `willPopInNavigationControllerWithAnimated:`、`didPopInNavigationControllerWithAnimated:` 这两个时机
 *
 *  @see NMUINavigationTitleView
 *  @see NMUIEmptyView
 */
@interface NMUICommonViewController : UIViewController {
    NMUIEmptyView *_emptyView;
}
...
/**
 *  NMUICommonViewController默认都会增加一个NMUINavigationTitleView的titleView，然后重写了setTitle来间接设置titleView的值。所以设置title的时候就跟系统的接口一样：self.title = xxx。
 *
 *  同时，NMUINavigationTitleView提供了更多的功能，具体可以参考NMUINavigationTitleView的文档。<br/>
 *  @see NMUINavigationTitleView
 */
@property(nullable, nonatomic, strong, readonly) NMUINavigationTitleView *titleView;
 ...
 */
@property(nullable, nonatomic, strong) NMUIEmptyView *emptyView;
...
@interface NMUICommonViewController (NMUISubclassingHooks)
...
@interface NMUICommonViewController (NMUINavigationController) <NMUINavigationControllerDelegate>

/**
 从 NMUINavigationControllerAppearanceDelegate 系列接口获取当前界面希望的导航栏样式并设置到导航栏上
 */
...
/**
 *  为了方便实现“点击空白区域降下键盘”的需求，NMUICommonViewController 内部集成一个 tap 手势对象并添加到 self.view 上，而业务只需要通过重写 -shouldHideKeyboardWhenTouchInView: 方法并根据当前被点击的 view 返回一个 BOOL 来控制键盘的显隐即可。
 *  @note 为了避免不必要的事件拦截，集成的手势 hideKeyboardTapGestureRecognizer：
 *  1. 默认的 enabled = NO。
 *  2. 如果当前 viewController 或其父类（非 NMUICommonViewController 那个层级的父类）没重写 -shouldHideKeyboardWhenTouchInView:，则永远 enabled = NO。
 *  3. 在键盘升起时，并且当前 viewController 重写了 -shouldHideKeyboardWhenTouchInView: 且处于可视状态下，此时手势的 enabled 才会被修改为 YES，并且在键盘消失时置为 NO。
 */
@interface NMUICommonViewController (NMUIKeyboard)
...
@property(nullable, nonatomic, strong, readonly) NMUIKeyboardManager *hideKeyboardManager;
