...
@interface NSObject (NMUIWindowSizeMonitor_Private)

@property(nonatomic, readonly) NSMutableArray <NMUIWindowSizeObserverHandler> *qwsm_windowSizeChangeHandlers;

@end

@interface UIResponder (NMUIWindowSizeMonitor_Private)
...
@interface UIWindow (NMUIWindowSizeMonitor_Private)
...
@implementation NSObject (NMUIWindowSizeMonitor)

- (void)nmui_addSizeObserverForMainWindow:(NMUIWindowSizeObserverHandler)handler {
    [self nmui_addSizeObserverForWindow:UIApplication.sharedApplication.delegate.window  handler:handler];
}

- (void)nmui_addSizeObserverForWindow:(UIWindow *)window handler:(NMUIWindowSizeObserverHandler)handler {
    ...
    for (NMUIWindowSizeObserverHandler handler in self.qwsm_windowSizeChangeHandlers) {
...
- (NSMutableArray<NMUIWindowSizeObserverHandler> *)qwsm_windowSizeChangeHandlers {
...
@implementation UIWindow (NMUIWindowSizeMonitor)

NMBFSynthesizeCGSizeProperty(qwsm_previousSzie, setQwsm_previousSzie)
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        NMBFExtendImplementationOfVoidMethodWithSingleArgument([UIWindow class], @selector(setFrame:), CGRect, notifyNewSizeBlock);
        
        NMBFExtendImplementationOfVoidMethodWithSingleArgument([UIWindow class], @selector(setBounds:), CGRect, notifyNewSizeBlock);
        
        NMBFOverrideImplementation([UIView class], @selector(willMoveToWindow:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
...
- (void)qwsm_addSizeObserver:(NSObject *)observer {
    if ([self.qwsm_sizeObservers nmbf_containsPointer:(__bridge void *)(observer)]) return;
    [self.qwsm_sizeObservers addPointer:(__bridge void *)(observer)];
}

- (void)qwsm_removeSizeObserver:(NSObject *)observer {
    NSUInteger index = [self.qwsm_sizeObservers nmbf_indexOfPointer:(__bridge void *)observer];
    if (index != NSNotFound) {
...
- (void)qwsm_addDidTransitionToSizeMethodReceiver:(UIResponder *)receiver {
    if ([self.qwsm_canReceiveWindowDidTransitionToSizeResponders nmbf_containsPointer:(__bridge void *)(receiver)]) return;
...
- (void)qwsm_removeDidTransitionToSizeMethodReceiver:(UIResponder *)receiver {
    NSUInteger index = [self.qwsm_canReceiveWindowDidTransitionToSizeResponders nmbf_indexOfPointer:(__bridge void *)(receiver)];
    if (index != NSNotFound) {
...
- (void)qwsm_notifyWithNewSize:(CGSize)newSize {
    ...
    for (NSUInteger i = 0, count = self.qwsm_sizeObservers.count; i < count; i++) {
        ...
        for (NSUInteger i = 0, count = object.qwsm_windowSizeChangeHandlers.count; i < count; i++) {
            NMUIWindowSizeObserverHandler handler = object.qwsm_windowSizeChangeHandlers[i];
            handler(newSize);
        }
    }
    ...
    for (NSUInteger i = 0, count = self.qwsm_canReceiveWindowDidTransitionToSizeResponders.count; i < count; i++) {
        UIResponder <NMUIWindowSizeMonitorProtocol>*responder = [self.qwsm_canReceiveWindowDidTransitionToSizeResponders pointerAtIndex:i];
...
@implementation UIResponder (NMUIWindowSizeMonitor)

NMBFSynthesizeIdWeakProperty(qwsm_previousWindow, setQwsm_previousWindow)
