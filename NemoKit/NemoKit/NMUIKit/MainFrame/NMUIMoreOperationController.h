...
@class NMUIMoreOperationController;
@class NMUIMoreOperationItemView;

@protocol NMUIMoreOperationControllerDelegate <NSObject>

@optional

/// 即将显示操作面板
- (void)willPresentMoreOperationController:(NMUIMoreOperationController *)moreOperationController;

/// 已经显示操作面板
- (void)didPresentMoreOperationController:(NMUIMoreOperationController *)moreOperationController;

/// 即将降下操作面板，cancelled参数是用来区分是否触发了maskView或者cancelButton按钮降下面板还是手动调用hide方法来降下面板。
- (void)willDismissMoreOperationController:(NMUIMoreOperationController *)moreOperationController cancelled:(BOOL)cancelled;

/// 已经降下操作面板，cancelled参数是用来区分是否触发了maskView或者cancelButton按钮降下面板还是手动调用hide方法来降下面板。
- (void)didDismissMoreOperationController:(NMUIMoreOperationController *)moreOperationController cancelled:(BOOL)cancelled;

/// itemView 点击事件，可以与 itemView.handler 共存，可通过 itemView.tag 或者 itemView.indexPath 来区分不同的 itemView
- (void)moreOperationController:(NMUIMoreOperationController *)moreOperationController didSelectItemView:(NMUIMoreOperationItemView *)itemView;
@end
 ...
 */
@interface NMUIMoreOperationController : UIViewController <NMUIModalPresentationContentViewControllerProtocol, NMUIModalPresentationViewControllerDelegate, NMUIModalPresentationComponentProtocol>
...
@property(nullable, nonatomic, weak) id<NMUIMoreOperationControllerDelegate> delegate;
...
@property(nullable, nonatomic, copy) NSArray<NSArray<NMUIMoreOperationItemView *> *> *items;
...
- (NMUIMoreOperationItemView * _Nullable)itemViewWithTag:(NSInteger)tag;

/// 获取指定 itemView 在当前控件里的 indexPath，如果不存在则返回 nil
- (NSIndexPath * _Nullable)indexPathWithItemView:(NMUIMoreOperationItemView *)itemView;
...
@interface NMUIMoreOperationController (UIAppearance)
...
@interface NMUIMoreOperationItemView : NMUIButton
...
+ (instancetype)itemViewWithImage:(UIImage * _Nullable)image
                          ...
                          handler:(void (^ _Nullable)(NMUIMoreOperationController *moreOperationController, NMUIMoreOperationItemView *itemView))handler;
