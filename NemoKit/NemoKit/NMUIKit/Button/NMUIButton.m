...
- (void)didInitialize {
    ...
    self.imagePosition = NMUIButtonImagePositionLeft;
}
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.imagePosition) {
        case NMUIButtonImagePositionTop:
        case NMUIButtonImagePositionBottom: {
        ...
        case NMUIButtonImagePositionLeft:
        case NMUIButtonImagePositionRight: {
            // 图片和文字水平排版时，高度以文字或图片的最大高度为最终高度
            // 注意这里有一个和系统不一致的行为：当 titleLabel 为多行时，系统的 sizeThatFits: 计算结果固定是单行的，所以当 NMUIButtonImagePositionLeft 并且titleLabel 多行的情况下，NMUIButton 计算的结果与系统不一致
...
- (void)layoutSubviews {
    ...
    if (self.imagePosition == NMUIButtonImagePositionTop || self.imagePosition == NMUIButtonImagePositionBottom) {
        ...
        if (self.imagePosition == NMUIButtonImagePositionTop) {
    ...
    } else if (self.imagePosition == NMUIButtonImagePositionLeft || self.imagePosition == NMUIButtonImagePositionRight) {
        ...
        if (self.imagePosition == NMUIButtonImagePositionLeft) {
            switch (self.contentHorizontalAlignment) {
                case UIControlContentHorizontalAlignmentLeft:
                    imageFrame = isImageViewShowing ? CGRectSetX(imageFrame, contentEdgeInsets.left + self.imageEdgeInsets.left) : imageFrame;
...
- (void)setImagePosition:(NMUIButtonImagePosition)imagePosition {
...
- (void)adjustsButtonHighlighted {
    if (self.highlightedBackgroundColor) {
        if (!self.highlightedBackgroundLayer) {
            ...
            [self.highlightedBackgroundLayer nmui_removeDefaultAnimations];
