...
@protocol NMUIKeyboardManagerDelegate;
@class NMUIKeyboardUserInfo;

/// 注意：由于某些Bug（例如 iOS 8 的 iPad 修改切换键盘类型，delegate 回调键盘高度值错误），NMUIKeyboardManager 不再支持 iPad 的浮动键盘了 - 更新于 2017.12.8 ///
/// 注意：NMUI 已经废弃 iOS8 了，所以浮动键盘又可以支持了

/**
 *  `NMUIKeyboardManager` 提供了方便管理键盘事件的方案，使用的场景是需要跟随键盘的显示或者隐藏来更改界面的 UI，例如输入框跟随在键盘的顶部。
 *  由于键盘通知是整个 App 全局的，所以经常会遇到 A 的键盘监听回调里接收到 B 的键盘事件，这样的情况往往不是我们想要的，即使可以通过判断当前的 firstResponder 来区分，但还是不能完美的解决问题或者有时候解决起来非常麻烦。`NMUIKeyboardManager` 通过 `delegateEnabled` 和 `targetResponder` 等属性来方便地控制 firstResponder，从而可以实现某个键盘监听回调方法只响应某个 UIResponder 或者某几个 UIResponder 触发的键盘通知。
 *  使用方式：
 ...
 *
 *  另外 NMUIKeyboardManager 同时集成在了 UITextField(NMUI) 和 UITextView(NMUI) 里，具体请查看对应文件。
 *  @see UITextField(NMUI)
 *  @see UITextView(NMUI)
 */
@interface NMUIKeyboardManager : NSObject

/**
 *  指定初始化方法，以 delegate 的方式将键盘事件传递给监听者
 */
- (instancetype)initWithDelegate:(id<NMUIKeyboardManagerDelegate>)delegate NS_DESIGNATED_INITIALIZER;

/**
 *  获取当前的 delegate
 */
@property(nonatomic, weak, readonly) id<NMUIKeyboardManagerDelegate> delegate;
...
@interface NMUIKeyboardUserInfo : NSObject

/**
 *  所在的KeyboardManager
 */
@property(nonatomic, weak, readonly) NMUIKeyboardManager *keyboardManager;
...
/**
 *  `NMUIKeyboardManagerDelegate`里面的方法是对应系统键盘通知的回调方法，具体请看delegate名字，`NMUIKeyboardUserInfo`是对系统的userInfo做了一个封装，可以方便的获取userInfo的属性值。
 */
@protocol NMUIKeyboardManagerDelegate <NSObject>
...
/// 持有KeyboardManager对象
@property(nonatomic, strong) NMUIKeyboardManager *nmui_keyboardManager;

@end

@interface UITextField (NMUI_KeyboardManager)

/// 键盘相关block，搭配NMUIKeyboardManager一起使用

@property(nonatomic, copy) void (^nmui_keyboardWillShowNotificationBlock)(NMUIKeyboardUserInfo *keyboardUserInfo);
@property(nonatomic, copy) void (^nmui_keyboardWillHideNotificationBlock)(NMUIKeyboardUserInfo *keyboardUserInfo);
@property(nonatomic, copy) void (^nmui_keyboardWillChangeFrameNotificationBlock)(NMUIKeyboardUserInfo *keyboardUserInfo);
@property(nonatomic, copy) void (^nmui_keyboardDidShowNotificationBlock)(NMUIKeyboardUserInfo *keyboardUserInfo);
@property(nonatomic, copy) void (^nmui_keyboardDidHideNotificationBlock)(NMUIKeyboardUserInfo *keyboardUserInfo);
@property(nonatomic, copy) void (^nmui_keyboardDidChangeFrameNotificationBlock)(NMUIKeyboardUserInfo *keyboardUserInfo);

@end

@interface UITextView (NMUI_KeyboardManager)
