...
//  NMNEmptyDataSet
//  https://github.com/NMenbot/NMNEmptyDataSet
//
//  Created by Ignacio Romero Zurbuchen on 6/20/14.
//  Copyright (c) 2016 NMN Labs. All rights reserved.
//  Licence: MIT-Licence
//
...
@interface UIView (NMNConstraintBasedLayoutExtensions)

- (NSLayoutConstraint *)equallyRelatedConstraintWithView:(UIView *)view attribute:(NSLayoutAttribute)attribute;

@end

@interface NMNWeakObjectContainer : NSObject
...
@interface NMNEmptyDataSetView : UIView
...
#define kEmptyImageViewAnimationKey @"com.NMn.emptyDataSet.imageViewAnimation"

@interface UIScrollView () <UIGestureRecognizerDelegate>
@property (nonatomic, readonly) NMNEmptyDataSetView *emptyDataSetView;
@end

@implementation UIScrollView (NMNEmptyDataSet)

#pragma mark - Getters (Public)

- (id<NMNEmptyDataSetSource>)emptyDataSetSource
{
    NMNWeakObjectContainer *container = objc_getAssociatedObject(self, kEmptyDataSetSource);
    return container.weakObject;
}

- (id<NMNEmptyDataSetDelegate>)emptyDataSetDelegate
{
    NMNWeakObjectContainer *container = objc_getAssociatedObject(self, kEmptyDataSetDelegate);
    return container.weakObject;
}
...
- (NMNEmptyDataSetView *)emptyDataSetView
{
    NMNEmptyDataSetView *view = objc_getAssociatedObject(self, kEmptyDataSetView);
    
    if (!view)
    {
        view = [NMNEmptyDataSetView new];
        view.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
        view.hidden = YES;
        
        view.tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(NMn_didTapContentView:)];
...
- (BOOL)NMn_canDisplay
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource conformsToProtocol:@protocol(NMNEmptyDataSetSource)]) {
...
- (NSAttributedString *)NMn_titleLabelString
{
...
- (NSAttributedString *)NMn_detailLabelString
{
...
- (CAAnimation *)NMn_imageAnimation
{
...
- (NSAttributedString *)NMn_buttonTitleForState:(UIControlState)state
{
...
- (UIImage *)NMn_buttonImageForState:(UIControlState)state
{
...
- (UIImage *)NMn_buttonBackgroundImageForState:(UIControlState)state
{
...
- (UIColor *)NMn_dataSetBackgroundColor
{
...
- (BOOL)NMn_shouldBeForcedToDisplay
{
...
- (BOOL)NMn_isImageViewAnimateAllowed
{
...
- (void)NMn_didTapContentView:(id)sender
{
...
- (void)NMn_didTapDataButton:(id)sender
{
...
- (void)setEmptyDataSetSource:(id<NMNEmptyDataSetSource>)datasource
{
    if (!datasource || ![self NMn_canDisplay]) {
        [self NMn_invalidate];
    }
    
    objc_setAssociatedObject(self, kEmptyDataSetSource, [[NMNWeakObjectContainer alloc] initWithWeakObject:datasource], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    
    // We add method sizzling for injecting -NMn_reloadData implementation to the native -reloadData implementation
    [self swizzleIfPossible:@selector(reloadData)];
    
    // Exclusively for UITableView, we also inject -NMn_reloadData to -endUpdates
    if ([self isKindOfClass:[UITableView class]]) {
        [self swizzleIfPossible:@selector(endUpdates)];
    }
}

- (void)setEmptyDataSetDelegate:(id<NMNEmptyDataSetDelegate>)delegate
{
    if (!delegate) {
        [self NMn_invalidate];
    }
    
    objc_setAssociatedObject(self, kEmptyDataSetDelegate, [[NMNWeakObjectContainer alloc] initWithWeakObject:delegate], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}


#pragma mark - Setters (Private)

- (void)setEmptyDataSetView:(NMNEmptyDataSetView *)view
{
...
{
    [self NMn_reloadEmptyDataSet];
}


#pragma mark - Reload APIs (Private)

- (void)NMn_reloadEmptyDataSet
{
    if (![self NMn_canDisplay]) {
        return;
    }
    
    if (([self NMn_shouldDisplay] && [self NMn_itemsCount] == 0) || [self NMn_shouldBeForcedToDisplay])
    {
        // Notifies that the empty dataset view will appear
        [self NMn_willAppear];
        
        NMNEmptyDataSetView *view = self.emptyDataSetView;
        ...
        UIView *customView = [self NMn_customView];
        ...
        if (customView) {
            ...
            NSAttributedString *titleLabelString = [self NMn_titleLabelString];
            NSAttributedString *detailLabelString = [self NMn_detailLabelString];
            
            UIImage *buttonImage = [self NMn_buttonImageForState:UIControlStateNormal];
            NSAttributedString *buttonTitle = [self NMn_buttonTitleForState:UIControlStateNormal];
            
            UIImage *image = [self NMn_image];
            UIColor *imageTintColor = [self NMn_imageTintColor];
            UIImageRenderingMode renderingMode = imageTintColor ? UIImageRenderingModeAlwaysTemplate : UIImageRenderingModeAlwaysOriginal;
            
            view.verticalSpace = [self NMn_verticalSpace];
            ...
            if (buttonImage) {
                ...
                [view.button setImage:[self NMn_buttonImageForState:UIControlStateHighlighted] forState:UIControlStateHighlighted];
            }
            else if (buttonTitle) {
                [view.button setAttributedTitle:buttonTitle forState:UIControlStateNormal];
                [view.button setAttributedTitle:[self NMn_buttonTitleForState:UIControlStateHighlighted] forState:UIControlStateHighlighted];
                [view.button setBackgroundImage:[self NMn_buttonBackgroundImageForState:UIControlStateNormal] forState:UIControlStateNormal];
                [view.button setBackgroundImage:[self NMn_buttonBackgroundImageForState:UIControlStateHighlighted] forState:UIControlStateHighlighted];
            }
        }
        
        // Configure offset
        view.verticalOffset = [self NMn_verticalOffset];
        
        // Configure the empty dataset view
        view.backgroundColor = [self NMn_dataSetBackgroundColor];
        view.hidden = NO;
        view.clipsToBounds = YES;
        
        // Configure empty dataset userInteraction permission
        view.userInteractionEnabled = [self NMn_isTouchAllowed];
        
        // Configure empty dataset fade in display
        view.fadeInOnDisplay = [self NMn_shouldFadeIn];
        ...
        self.scrollEnabled = [self NMn_isScrollAllowed];
        
        // Configure image view animation
        if ([self NMn_isImageViewAnimateAllowed])
        {
            CAAnimation *animation = [self NMn_imageAnimation];
...
static NSString *const NMNSwizzleInfoPointerKey = @"pointer";
static NSString *const NMNSwizzleInfoOwnerKey = @"owner";
static NSString *const NMNSwizzleInfoSelectorKey = @"selector";
...
void NMn_original_implementation(id self, SEL _cmd)
{
    // Fetch original implementation from lookup table
    Class baseClass = NMn_baseClassToSwizzleForTarget(self);
    NSString *key = NMn_implementationKey(baseClass, _cmd);
    
    NSDictionary *swizzleInfo = [_impLookupTable objectForKey:key];
    NSValue *impValue = [swizzleInfo valueForKey:NMNSwizzleInfoPointerKey];
...
NSString *NMn_implementationKey(Class class, SEL selector)
{
    if (!class || !selector) {
        return nil;
    }
...
Class NMn_baseClassToSwizzleForTarget(id target)
{
...
- (void)swizzleIfPossible:(SEL)selector
{
    ...
    for (NSDictionary *info in [_impLookupTable allValues]) {
        Class class = [info objectForKey:NMNSwizzleInfoOwnerKey];
        NSString *selectorName = [info objectForKey:NMNSwizzleInfoSelectorKey];
    ...
    Class baseClass = NMn_baseClassToSwizzleForTarget(self);
    NSString *key = NMn_implementationKey(baseClass, selector);
    NSValue *impValue = [[_impLookupTable objectForKey:key] valueForKey:NMNSwizzleInfoPointerKey];
    ...
    IMP NMn_newImplementation = method_setImplementation(method, (IMP)NMn_original_implementation);
    
    // Store the new implementation in the lookup table
    NSDictionary *swizzledInfo = @{NMNSwizzleInfoOwnerKey: baseClass,
                                   NMNSwizzleInfoSelectorKey: NSStringFromSelector(selector),
                                   NMNSwizzleInfoPointerKey: [NSValue valueWithPointer:NMn_newImplementation]};
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer
{
    if ([gestureRecognizer.view isEqual:self.emptyDataSetView]) {
        return [self NMn_isTouchAllowed];
    }
...
#pragma mark - NMNEmptyDataSetView

@interface NMNEmptyDataSetView ()
@end

@implementation NMNEmptyDataSetView
...
{
    SEL selector = NSSelectorFromString(@"NMn_didTapDataButton:");
...
#pragma mark - UIView+NMNConstraintBasedLayoutExtensions

@implementation UIView (NMNConstraintBasedLayoutExtensions)
...
#pragma mark - NMNWeakObjectContainer

@implementation NMNWeakObjectContainer
